...
app.post("/api/referral/status", async (req, res) => {
  try {
    ...
    const idx = thresholds.findIndex((t) => totalInvites < t);
    const nextAt = idx === -1 ? thresholds[thresholds.length - 1] : thresholds[idx];
    const prevAt = idx <= 0 ? 0 : thresholds[idx - 1];
...
// Optional GET variant for manual testing / backwards compatibility
