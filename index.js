...
app.post("/api/boost/energy", async (req, res) => {
  try {
    ...
    if (method === "points") {
      ...
      // Atomic + NULL-safe: refill only if the user can afford it and is not already full.
      // (Some legacy rows may have NULL energy; treat that as 0 for comparisons.)
      const upd = await pool.query(
        `
        UPDATE public.users
        SET balance        = balance - $1,
        ...
        WHERE id = $2
          AND balance >= $1
          AND COALESCE(energy, 0) < COALESCE(max_energy, 50)
        RETURNING *;
        `,
      ...
      if (!enforceSponsor) {
        const upd = await pool.query(
          `
          UPDATE public.users
          SET energy         = COALESCE(max_energy, 50),
              last_energy_ts = NOW()
          WHERE id = $1
          RETURNING *;
          `,
    ...
    // If the atomic UPDATE affected no rows (e.g., already full, insufficient points, or a race),
    // return a clear reason and do NOT pretend the refill happened.
    if (!updatedUser) {
      const fresh = await pool.query(`SELECT * FROM public.users WHERE id = $1 LIMIT 1;`, [user.id]);
      const u = fresh.rows[0] || user;
      const maxE = Number(u.max_energy || 50);
      const curE = Number(u.energy || 0);
      const curB = Number(u.balance || 0);
      const state = await buildClientState(u);
      return res.json({
        ...state,
        ok: false,
        reason: curE >= maxE ? "ENERGY_FULL" : curB < ENERGY_REFILL_COST ? "NOT_ENOUGH_POINTS" : "REFILL_NOT_APPLIED",
      });
    }
  ...
  try {
    ...
    if (method === "points") {
      ...
      const upd = await pool.query(
        ...
        UPDATE public.users
        SET balance = balance - $1,
            double_boost_until = NOW() + INTERVAL '10 minutes'
        WHERE id = $2
          AND balance >= $1
          AND (double_boost_until IS NULL OR double_boost_until < NOW())
        RETURNING *;
        `,
