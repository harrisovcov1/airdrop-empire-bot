...
async function getOrCreateUserFromInitData(req) {
  ...
  if (!telegramUserId) {
    const e = new Error("NO_TELEGRAM_ID");
...
app.post("/api/state", async (req, res) => {
  try {
    ...
    if (err && (err.code === "NO_TELEGRAM_ID" || err.message === "NO_TELEGRAM_ID")) {
      return res.status(200).json({ ok: false, reason: "NO_TELEGRAM_ID" });
    }
...
app.post("/api/tapPacket", async (req, res) => {
  try {
    ...
    const fp = req._fp || getClientFingerprint(req);
    // Allow higher request cadence because the client batches taps (packets)
    // and may flush rapidly when the app is backgrounded/foregrounded.
    // The real anti-abuse cap remains: TAP_PACKET_MAX + energy + daily cap.
    if (!hit(`tapburst:${fp}`, 20, 1000)) {
      return res.status(429).json({ ok: false, error: "TAP_RATE_LIMIT" });
    }
