...
async function sponsorChargeIfNeeded(user, mission) {
  ...
  if (!camp.is_active) {
  // Allow claims even if campaign is inactive (do not block user rewards).
  // We still record the event for auditing.
  await pool.query(
    `INSERT INTO public.sponsor_ledger (campaign_id, user_id, mission_id, event, amount, meta)
     VALUES ($1,$2,$3,'claim',0,$4)`,
    [camp.id, user.id, mission.id, { note: "campaign_inactive" }]
  );
  return { charged: false, note: "campaign_inactive", campaign_id: camp.id };
}

  const cpc = Number(camp.cpc || 0);
  const remaining = Number(camp.budget_remaining || 0);
  if (cpc > 0 && remaining < cpc) {
  // Allow claims even if campaign budget is exhausted (do not block user rewards).
  await pool.query(
    `INSERT INTO public.sponsor_ledger (campaign_id, user_id, mission_id, event, amount, meta)
     VALUES ($1,$2,$3,'claim',0,$4)`,
    [camp.id, user.id, mission.id, { note: "budget_exhausted" }]
  );
  return { charged: false, note: "budget_exhausted", campaign_id: camp.id };
}
...
app.post("/api/mission/complete", async (req, res) => {
  ...
  if (isSponsorMission) {
    // For sponsor quests we allow a direct Claim path (some clients get suspended before "start" persists).
    // We create/refresh a started_at that satisfies the min wait so Claim can succeed.
    const minS = Number(mission.min_seconds_to_claim || 30);
    await pool.query(
      `
      INSERT INTO public.user_missions (user_id, mission_id, status, started_at, claimed_at, reward_applied)
      VALUES ($1, $2, 'started', NOW() - ($3 || ' seconds')::interval, NULL, FALSE)
      ON CONFLICT (user_id, mission_id)
      DO UPDATE SET
        status = 'started',
        started_at = COALESCE(public.user_missions.started_at, NOW() - ($3 || ' seconds')::interval),
        claimed_at = NULL,
        reward_applied = COALESCE(public.user_missions.reward_applied, FALSE);
      `,
      [user.id, mission.id, `${minS}`]
    );
  } else {
...
// Apply reward once (sponsor missions are granted immediately on Claim in this build).
// Apply reward once
if (!um.reward_applied) {
