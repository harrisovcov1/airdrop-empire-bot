...
async function getOrCreateUserFromInitData(req) {
  // Telegram can deliver initData in body, query, or headers depending on platform/webview.
  // Support all common variants to avoid creating "new" users (balance=0) on iOS sessions.
  const initDataRaw =
    req.body.initData ||
    req.query.initData ||
...
async function applyEnergyRegen(user) {
  ...
  // If we didn't gain any energy, don't move the regen cursor.
  const newLastTs = gained > 0 ? new Date(advancedMs) : last;
