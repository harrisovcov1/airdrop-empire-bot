...
app.get("/api/shop/starter-offer", async (req, res) => {
  try {
    ...
    const { rows: userRows } = await pool.query(
      `
      select id, created_at, is_system_user
      from public.users
    ...
    const { rows: productRows } = await pool.query(
      `
      select pack_sku, coins_granted, currency, amount_minor, checkout_url
      from public.payment_products
      where provider = 'coinbase'
        and pack_sku = $1
        and active = true
    ...
    if (productRows.length === 0 || !productRows[0].checkout_url) {
      return res.json({
        success: true,
        available: false,
        reason: "no_product",
    ...
    const alreadyPurchased = existingRows.length > 0;

    const product = productRows[0];
    const currency = product.currency || "GBP";
    const amountMinor = product.amount_minor || 0;
    const coinsGranted = product.coins_granted || 0;
