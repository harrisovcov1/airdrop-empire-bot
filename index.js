...
app.post("/api/boost/energy", async (req, res) => {
  try {
    ...
    if (method === "points") {
      ...
      // Sponsor path: require the user to have completed+claimed the sponsor mission
    ...
    return res.json({
      ...
      message:
        method === "points"
          ...
          : "⚡ Energy refilled – sponsor completed.",
    });
  } catch (err) {
...
app.post("/api/boost/double", async (req, res) => {
  try {
    ...
    if (method === "points") {
      ...
      // Sponsor path: user must complete the sponsor mission tied to this boost.
