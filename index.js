...
async function ensureSchema(client) {
  ...
  // Missions v2/v3 fields (safe additive schema)
  await client.query(`
    ...
    ADD COLUMN IF NOT EXISTS reward_value INT,
    -- Sponsor economics (optional)
    ADD COLUMN IF NOT EXISTS sponsor_cpc NUMERIC,
    ADD COLUMN IF NOT EXISTS sponsor_daily_cap INT,
    ADD COLUMN IF NOT EXISTS sponsor_claims_today INT DEFAULT 0,
    ADD COLUMN IF NOT EXISTS profit_per_claim NUMERIC,
    ADD COLUMN IF NOT EXISTS sponsor_budget_remaining NUMERIC,
    ADD COLUMN IF NOT EXISTS sponsor_active BOOLEAN DEFAULT FALSE;
  `);

  // Optional integrity: if a mission is sponsor_active it must have reward fields populated
  await client.query(`
    DO $$
    BEGIN
      IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'missions_active_requires_reward_fields'
      ) THEN
        ALTER TABLE public.missions
        ADD CONSTRAINT missions_active_requires_reward_fields
        CHECK (
...
async function sponsorChargeIfNeeded(user, mission) {
  ...
  // Track on missions table too (optional)
  await pool.query(
    `
    UPDATE public.missions
    SET sponsor_claims_today = COALESCE(sponsor_claims_today,0) + 1,
        sponsor_budget_remaining = CASE
          WHEN sponsor_budget_remaining IS NULL THEN NULL
          ELSE GREATEST(0, sponsor_budget_remaining - $1)
        END
    WHERE id = $2;
    `,
    [cpc, mission.id]
...
<title>Airdrop Empire Admin</title>
<style>
...
/**
 * Admin: reset sponsor missions daily counters + rotate ordering.
 * Call with header: x-admin-key: <ADMIN_KEY>
 */
app.post("/api/admin/sponsors/dailyReset", async (req, res) => {
  try {
    if (!requireAdmin(req, res)) return;

    await pool.query(`UPDATE public.missions SET sponsor_claims_today = 0 WHERE kind = 'sponsor';`);

    await pool.query(`
      UPDATE public.missions
      SET sponsor_active = FALSE
      WHERE kind = 'sponsor'
        AND sponsor_budget_remaining IS NOT NULL
        AND sponsor_budget_remaining <= 0;
    `);
    ...
    await pool.query(`
      WITH s AS (
        ...
        WHERE kind = 'sponsor' AND sponsor_active = TRUE
      )
  ...
  try {
    ...
    return res.json({
      ...
      vip: {
        ...
        vip_until: vipUntil && !isNaN(vipUntil) ? vipUntil.toISOString() : null,
        perks: {
...
app.post("/api/mission/list", async (req, res) => {
  try {
    ...
    const sponsorOnly = kind && String(kind).toLowerCase() === "sponsor";

    const params = [];
    let where = "WHERE is_active = TRUE";
    if (sponsorOnly) {
      where += " AND sponsor_active = TRUE AND (sponsor_budget_remaining IS NULL OR sponsor_budget_remaining > 0)";
    }
    if (kind) {
      params.push(kind);
    ...
    const missionsRes = await pool.query(
      ...
      SELECT id, code, title, description, payout_type, payout_amount, url, kind,
             reward_type, reward_value, sort_order, sponsor_active, sponsor_cpc, sponsor_daily_cap, sponsor_claims_today, sponsor_budget_remaining,
...
app.post("/api/mission/complete", async (req, res) => {
  try {
    ...
    if (!um.reward_applied) {
      ...
      // Determine reward (supports both legacy payout_* and new reward_*)
      const rTypeRaw = (mission.reward_type || mission.payout_type || "").toString().toLowerCase();
      const rValueRaw = mission.reward_value ?? mission.payout_amount ?? 0;
      const rValue = Number(rValueRaw || 0);

      // Apply reward to user
      if (rTypeRaw === "points" || rTypeRaw === "balance") {
        if (rValue > 0) {
          const updUser = await pool.query(
            `UPDATE public.users SET balance = COALESCE(balance,0) + $1 WHERE id = $2 RETURNING *;`,
            [rValue, user.id]
          );
          user = updUser.rows[0];
        }
      } else if (rTypeRaw === "energy") {
        if (rValue > 0) {
          const updUser = await pool.query(
            `
            UPDATE public.users
            SET energy = LEAST(COALESCE(energy_max,50), COALESCE(energy,0) + $1)
            WHERE id = $2
            RETURNING *;
            `,
            [rValue, user.id]
      ...
      } else if (rTypeRaw.startsWith("double")) {
        // give 10 minutes double boost by default
        const updUser = await pool.query(
          `
          UPDATE public.users
          SET double_boost_until = GREATEST(COALESCE(double_boost_until, NOW()), NOW() + ('10 minutes')::interval)
          WHERE id = $1
          RETURNING *;
          `,
          [user.id]
        ...
        console.warn("Unknown mission reward_type:", rTypeRaw, "for mission", mission.code);
      }
