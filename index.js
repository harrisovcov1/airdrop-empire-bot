...
// Starter balance for brand-new users.
// NOTE: Existing users should be handled via a one-time Supabase SQL migration.
const STARTER_BALANCE = Number(process.env.STARTER_BALANCE || 5000);
const EARLY_ACCESS_CAP = Number(process.env.EARLY_ACCESS_CAP || 1000);
...
app.get("/api/stats", async (_req, res) => {
  try {
    ...
    const limit = 1000;
    const remaining = Math.max(0, limit - totalUsers);
    const now = Date.now();
    ...
    return res.json({
      ...
      early_bonus_active: totalUsers < limit && timeOk,
...
app.post("/api/coinbase/webhook", async (req, res) => {
  try {
    ...
    const eventId = event && (event.id || (event.event && event.event.id));
    try {
    ...
    // We credit on confirmed or resolved (resolved is final). Accept both for safety.
    if (type === "charge:confirmed" || type === "charge:resolved") {
      const charge = event && event.data;
      const meta = (charge && charge.metadata) ? charge.metadata : {};
      const telegramId = Number(meta.telegram_id || 0);
      const sku = String(meta.sku || "").trim();
      const coinsGranted = meta.coins_granted != null ? Number(meta.coins_granted) : null;
...
async function ensureSchema(client) {
  ...
  await client.query(`
    ...
    ADD COLUMN IF NOT EXISTS telegram_id BIGINT
        ,ADD COLUMN IF NOT EXISTS early_access_awarded BOOLEAN DEFAULT FALSE
        ,ADD COLUMN IF NOT EXISTS early_access_rank INT
        ,ADD COLUMN IF NOT EXISTS early_access_awarded_at TIMESTAMPTZ;
...
async function getOrCreateUserFromInitData(req) {
  ...
  if (!parsed || !parsed.user) throw new Error('Invalid initData');

  // Telegram sends `user` as a JSON string in initData. Normalize it to an object.
  let userObj = parsed.user;
  if (typeof userObj === 'string') {
    try { userObj = JSON.parse(userObj); } catch (e) { /* ignore */ }
  }
  if (!userObj || typeof userObj !== 'object' || userObj.id == null) {
    const err = new Error('Invalid initData: missing user.id');
    err.statusCode = 400;
    throw err;
  }

  const telegramUserId = String(userObj.id);
  const username = userObj.username ? String(userObj.username).toLowerCase() : null;
  const firstName = userObj.first_name ? String(userObj.first_name) : null;
  const lastName = userObj.last_name ? String(userObj.last_name) : null;
  const photoUrl = userObj.photo_url ? String(userObj.photo_url) : null;
  ...
  try {
    ...
    if (existing.rows.length) {
      // Keep balance; just refresh profile fields.
      await client.query(
        `UPDATE public.users
         SET username = $2,
             first_name = $3,
    ...
    // Server-side Early Access cap: only the first EARLY_ACCESS_CAP users receive STARTER_BALANCE.
    // Locking prevents race conditions where multiple signups would exceed the cap.
    await client.query('LOCK TABLE public.users IN EXCLUSIVE MODE');

    const countRes = await client.query('SELECT COUNT(*)::int AS count FROM public.users');
    const currentCount = countRes.rows[0].count;

    const earlyEligible = currentCount < EARLY_ACCESS_CAP;
    const starter = earlyEligible ? STARTER_BALANCE : 0;
    const earlyRank = earlyEligible ? (currentCount + 1) : null;

    const insertRes = await client.query(
      `INSERT INTO public.users
        (telegram_id, username, first_name, last_name, photo_url, balance, early_access_awarded, early_access_rank, early_access_awarded_at)
       VALUES
        ($1, $2, $3, $4, $5, $6, $7, $8, CASE WHEN $7 THEN NOW() ELSE NULL END)
       RETURNING *`,
      [telegramUserId, username, firstName, lastName, photoUrl, starter, earlyEligible, earlyRank]
    );
...
if (isSponsorMission) {
  // Sponsor missions are verified via server-to-server postback (/postback/ogads).
  // Mark as pending and do NOT grant rewards here.
  await pool.query(
    `
    UPDATE public.user_missions
    SET status = 'pending',
  ...
  await logEvent(user.id, "mission_pending_verification", { code: mission.code, kind: mission.kind || null });
  const state = await buildClientState(user);
  return res.json({
    ...state,
    ok: true,
    mission: { code: mission.code, status: "pending", reward_applied: false },
...
bot.start(async (ctx) => {
  try {
    ...
    try {
      ...
      // Create/update user, applying server-side Early Access cap for the starter balance
      ...
      if (existingRes.rows.length) {
        ...
        const insertRes = await client.query(
          `INSERT INTO public.users
            (telegram_id, username, first_name, last_name, language_code, balance, early_access_awarded, early_access_rank, early_access_awarded_at)
           VALUES
            ($1,$2,$3,$4,$5,$6,$7,$8, CASE WHEN $7 THEN NOW() ELSE NULL END)
           RETURNING *`,
          [telegramId, username, firstName, lastName, languageCode, starter, earlyEligible, earlyRank]
        );
    ...
    // Try to pin a persistent chat menu button (improves conversion).
    try {
    ...
    const welcomeMsg =
      `🔥 *JigCoin — Early Access Live*\n\n` +
      `✅ New players start with *${STARTER_BALANCE.toLocaleString()}* coins\n` +
      `👥 Invite friends: *+${REFERRAL_REWARD}* coins each (higher early)\n` +
      `🏆 Climb *Global Rank* to lock stronger token allocation\n\n` +
