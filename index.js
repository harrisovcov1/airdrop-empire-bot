...
// Rewarded-ad mapping (used by the "Watch Ad" option on boosts)
// These mission codes should exist in `public.missions` with kind='sponsor' and url set to your SmartLink.
// If they don't exist, we fall back to the generic sponsor mission (sp_watch_video) so nothing breaks.
async function getSponsorMissionCodeForRewardType(rewardType) {
  const candidatesByType = {
    // Your two NEW rows you said you created for the 2 boost buttons:
    energy_refill: ["sp_energy_refill", "sp_energy_boost_ad", "sp_watch_video"],
    double_10m: ["sp_double_points", "sp_double_points_ad", "sp_watch_video"],

    // Other ad-reward types we already support:
    points: ["sp_bonus_points", "sp_watch_video"],
    energy_boost: ["sp_energy_boost", "sp_watch_video"],
    // default fallback
    _default: ["sp_watch_video"],
  };

  const list = candidatesByType[rewardType] || candidatesByType._default;
  for (const code of list) {
    const m = await getMissionByCode(code);
    if (m && m.kind === "sponsor") return code;
  }
  return "sp_watch_video";
}
...
app.post("/api/boost/completeAd", async (req, res) => {
  try {
    ...
    // IMPORTANT: the boost buttons should charge their own sponsor mission rows (not the same as the 5 sponsor quests),
    // so you can measure / rotate / cap them separately.
    try {
      const sponsorMissionCode = await getSponsorMissionCodeForRewardType(ad.reward_type);
      if (sponsorMissionCode) {
        await sponsorChargeIfNeeded(user, sponsorMissionCode);
      }
    } catch (e) {
