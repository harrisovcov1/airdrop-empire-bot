...
app.post("/api/task", async (req, res) => {
  try {
    ...
    // 1) Credit the daily reward via the ledger
    await applyBalanceChange({
      userId: user.id,
      delta: totalReward,
      reason: "daily_bonus",
    ...
    // 2) Update streak + timestamps without touching balance
    const upd = await pool.query(
      `
      UPDATE public.users
      SET last_daily = $1,
...
app.post("/api/withdraw/request", async (req, res) => {
  try {
    ...
    // Reserve funds immediately via the balance ledger (prevents drain)
    const wr = await withTransaction(async (client) => {
      // 1) Reserve the funds on the balance ledger
      await applyBalanceChangeTx(client, {
        userId: user.id,
        delta: -amount,
      ...
      // 2) Insert the withdraw request row bound to this reservation
