  ...
  try {
    ...
    try {
      ...
      if (bonusRes && bonusRes.awarded) {
        ...
        user.balance = Number(user.balance || 0) + Number(bonusRes.bonus || 0);
      }
    } catch (e) {
...
async function maybeAwardEarlyAccessStarterBonus(client, userId, rank) {
  const CAP = Number(process.env.EARLY_ACCESS_CAP || 1000);
  const BONUS = Number(process.env.EARLY_ACCESS_STARTER_BONUS || 5000);

  // Only first CAP ranks are eligible
  if (!Number.isFinite(rank) || rank <= 0 || rank > CAP) {
    return { awarded: false, reason: "not_eligible_rank" };
  }
  ...
  try {
    // Hold the lock for the whole transaction (xact lock only works if we keep a transaction open)
    ...
    const issued_before = issued.rows?.[0]?.n ?? 0;

    // Sanity: don't exceed CAP awards even if rank is somehow within CAP
    ...
    await client.query(
      `INSERT INTO public.event_log (user_id, event, meta, created_at)
       VALUES ($1, 'early_access_starter_bonus', jsonb_build_object('rank', $2, 'amount', $3, 'before', $4, 'after', $5, 'issued_before', $6), NOW())`,
      [userId, rank, BONUS, before, after, issued_before]
    );

    await client.query("COMMIT");
    return { awarded: true, amount: BONUS, rank, balance_after: after };
  } catch (e) {
