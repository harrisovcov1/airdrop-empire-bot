...
const STARTER_PACK_SKU = String(process.env.STARTER_PACK_SKU || "STARTER_1").trim();
const STARTER_OFFER_WINDOW_MIN = Number(process.env.STARTER_OFFER_WINDOW_MIN || 60);
...
// Returns one-time starter offer details for eligible users.
// Uses Coinbase payment_products (configured by SKU via STARTER_PACK_SKU).
app.post("/api/shop/starter-offer", async (req, res) => {
  try {
    const fp = req._fp || getClientFingerprint(req);
    if (!hit(`starter:${fp}`, 20, 60_000)) {
      return res.status(429).json({ ok: false, error: "RATE_LIMIT" });
    }
    ...
    if (!user.has_completed_onboarding) {
      return res.json({ ok: true, available: false, reason: "ONBOARDING_INCOMPLETE" });
    }

    if (!STARTER_PACK_SKU) {
      return res.json({ ok: true, available: false, reason: "NOT_CONFIGURED" });
    }

    // Check if this user has already purchased the starter SKU (idempotent).
    ...
    try {
      const { rows: plRows } = await pool.query(
        ...
        LIMIT 1;
        `,
        [String(user.telegram_id), STARTER_PACK_SKU]
      );
      alreadyPurchased = plRows.length > 0;
    } catch (e) {
      console.error("Error checking starter purchase_ledger:", e);
    }

    if (alreadyPurchased) {
      return res.json({ ok: true, available: false, reason: "ALREADY_PURCHASED" });
    }

    // Look up the configured starter product in payment_products.
    ...
    const p = prodRows[0];
    const amountMinor = p.amount_minor == null ? null : Number(p.amount_minor);
    const priceGbp = amountMinor == null ? null : amountMinor / 100;

    // Time-limited window: first time the offer is requested, start countdown.
    const windowMinutes = Number.isFinite(STARTER_OFFER_WINDOW_MIN) && STARTER_OFFER_WINDOW_MIN > 0
      ? STARTER_OFFER_WINDOW_MIN
      : 60;

    let seenAt = user.starter_offer_seen_at;
    if (!seenAt) {
      try {
        const { rows: uRows } = await pool.query(
          `
          UPDATE public.users
          SET starter_offer_seen_at = NOW()
          WHERE id = $1
          RETURNING starter_offer_seen_at;
          `,
          [user.id]
        );
        if (uRows.length) {
          seenAt = uRows[0].starter_offer_seen_at;
        }
      } catch (e) {
        console.error("Error setting starter_offer_seen_at:", e);
      }
    }

    let expiresAtIso = null;
    let expiresIn = null;
    if (seenAt && windowMinutes > 0) {
      const seenMs = new Date(seenAt).getTime();
      const expMs = seenMs + windowMinutes * 60 * 1000;
      const nowMs = Date.now();
      if (expMs <= nowMs) {
        return res.json({ ok: true, available: false, reason: "EXPIRED" });
      }
      expiresIn = Math.max(0, Math.floor((expMs - nowMs) / 1000));
    ...
    return res.json({
      ...
      product: {
        sku: p.pack_sku,
        title: p.title || "Starter Boost Bundle",
    ...
    console.error("Error /api/shop/starter-offer:", err);
    return res.status(500).json({ ok: false, error: "STARTER_OFFER_ERROR" });
  }
});
...
async function ensureSchema(client) {
  ...
  await client.query(`
    ALTER TABLE public.users
    ADD COLUMN IF NOT EXISTS starter_offer_seen_at TIMESTAMPTZ;
  `);
...
async function getGlobalRankForUser(user) {
  ...
  const totalRes = await pool.query(`SELECT COUNT(*) AS count FROM public.users WHERE NOT is_system_user;`);
