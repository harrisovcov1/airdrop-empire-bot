...
// ------------ Stripe Webhook Idempotency (PRIVATE schema) ------------
// We store every Stripe event exactly once in private.stripe_webhook_events.
// This protects against Stripe retries and prevents double-crediting.
async function recordStripeEvent(event) {
  if (!event || !event.id) return { ok: false, inserted: false };

  try {
    const payload = event; // already a plain object from Stripe SDK
    const r = await pool.query(
      `
      INSERT INTO private.stripe_webhook_events
        (event_id, event_type, livemode, payload, status, received_at)
      VALUES
        ($1, $2, $3, $4::jsonb, 'received', NOW())
      ON CONFLICT (event_id) DO NOTHING
      RETURNING event_id;
      `,
      [String(event.id), String(event.type || 'unknown'), !!event.livemode, JSON.stringify(payload)]
    );
    return { ok: true, inserted: r.rowCount > 0 };
  } catch (e) {
    // If this fails, we still want Stripe to retry rather than silently lose an event.
    console.error("recordStripeEvent failed:", e.message);
    return { ok: false, inserted: false };
  }
}

async function setStripeEventStatus(eventId, status, errorMessage = null) {
  if (!eventId) return;
  try {
    await pool.query(
      `
      ...
      WHERE event_id = $1;
      `,
      [String(eventId), String(status), errorMessage ? String(errorMessage).slice(0, 2000) : null]
    );
  } catch (e) {
    console.error("setStripeEventStatus failed:", e.message);
  }
}
}
...
// ------------ Shop (Coin Packs) ------------
// Phase 2: Read-only endpoint to serve live coin packs from Supabase
// NOTE: This intentionally does NOT change any existing gameplay logic.
  ...
  try {
    ...
    const { rows } = await pool.query(
      ...
      WHERE COALESCE(is_active, active, TRUE) = TRUE
        ...
        AND TRIM(stripe_payment_link) <> ''
      ORDER BY sort_order ASC NULLS LAST, price_gbp ASC NULLS LAST, id ASC;
      `
    );
    ...
    const packs = rows.map((r) => ({
      ...
      coins: r.coins == null ? 0 : Number(r.coins),
      bonus_coins: r.bonus_coins == null ? 0 : Number(r.bonus_coins),
...
// ------------ VIP Shop (Stripe) ------------
// A) Read active VIP product config from Supabase (vip_products)
...
app.post("/api/stripe/webhook", async (req, res) => {
  try {
    ...
    // Idempotency: record each Stripe event once (Stripe will retry on non-2xx)
    const rec = await recordStripeEvent(event);
    if (!rec.ok) {
      return res.status(500).send("idempotency_error");
    }
    if (!rec.inserted) {
      // Already received/processed earlier (or being processed)
    ...
    if (event.type === "checkout.session.completed") {
      ...
      const durationDays = Math.max(1, Math.min(365, Number((session.metadata && session.metadata.duration_days) || 30)));
      if (telegramId) {
        // Extend VIP from the later of (now) or (existing vip_until)
        await pool.query(
          `
          ...
          DO UPDATE SET
            ...
            updated_at = NOW();
          `,
          [telegramId, durationDays]
        );

        // --- Insert purchase + ledger (audit trail) ---
        try {
          // Look up internal user id / balances (for audit fields)
          const { rows: userRows } = await pool.query(
            `SELECT id, balance, energy FROM public.users WHERE telegram_id = $1 LIMIT 1`,
            [telegramId]
          );
          const userId = userRows?.[0]?.id ?? null;
          const balanceAfter = userRows?.[0]?.balance ?? null;
          const energyAfter = userRows?.[0]?.energy ?? null;
          ...
          const productType = "vip";
          const productId = `vip_${durationDays}d`;
          const status = typeof session.payment_status === "string" ? session.payment_status : "paid";
          ...
          // Insert into purchases (best-effort, idempotent)
          if (userId !== null) {
            await pool.query(
              `INSERT INTO public.purchases
                (user_id, stripe_event_id, stripe_session_id, stripe_payment_intent_id, product_type, product_id, amount_cents, currency, status, metadata)
               VALUES
          ...
          // Get purchase id (if inserted previously or just now)
          const { rows: purchaseRows } = await pool.query(
            `SELECT id FROM public.purchases WHERE stripe_session_id = $1 LIMIT 1`,
            [stripeSessionId]
          );
          const purchaseId = purchaseRows?.[0]?.id ?? null;

          // Insert into purchase_ledger (best-effort; user_id may be nullable depending on your schema)
          if (purchaseId !== null) {
            await pool.query(
              `INSERT INTO public.purchase_ledger
                (purchase_id, stripe_event_id, stripe_session_id, type, amount_cents, currency, delta_balance, delta_energy, balance_after, metadata, user_id)
               VALUES
              ...
              [
                ...
                0,
                balanceAfter,
                JSON.stringify({ ...metadata, energy_after: energyAfter, balance_after: balanceAfter }),
                null,
              ]
            );
          }
        } catch (e) {
          console.error("Purchase insert/ledger failed (non-fatal):", e);
        }
    ...
    await setStripeEventStatus(event.id, handled ? "processed" : "ignored");
    return res.status(200).send("ok");
  } catch (err) {
    console.error("Error /api/stripe/webhook:", err);
    try {
      if (typeof event !== "undefined" && event && event.id) {
        await setStripeEventStatus(event.id, "error", err && err.message ? err.message : String(err));
      }
    } catch (e) {}
...
app.post("/api/mission/start", async (req, res) => {
  try {
    ...
    res.json({ ok: true, code: mission.code, redirect_url: mission.url, min_seconds_to_claim: Number(mission.min_seconds_to_claim || 30) });
  } catch (err) {
