...
async function getOrCreateUserFromInitData(req) {
  const initDataRaw = req.body.initData || req.query.initData || req.headers["x-telegram-initdata"] || req.headers["x-telegram-init-data"] || "";
...
async function applyEnergyRegen(user) {
  ...
  // To keep gameplay pacing consistent (and avoid "instant" refills on state polls),
  // cap how many energy points can be regenerated per API call.
  // This makes refill feel gradual in-app (better for retention/revenue loops).
  const MAX_GAIN_PER_CALL = Number(process.env.ENERGY_REGEN_MAX_PER_CALL || 4);

  // If never had regen timestamp, start from now (no offline banking)
...
const last = new Date(user.last_energy_ts);
  ...
  // Advance an internal timestamp by the amount of time we "spent" regenerating.
  // This prevents banking a huge offline gap into an instant refill.
  let advancedMs = last.getTime();
  let gained = 0;

  while (diffSeconds > 0 && energy < maxEnergy && gained < MAX_GAIN_PER_CALL) {
    let step;

    // Hybrid model: low energy refills faster, slows as it approaches full.
    // Tuned slower than before to avoid refill overpowering tapping.
    if (energy < 10) step = 2;       // fast regen (was 1)
    else if (energy < 30) step = 6;  // medium regen (was 3)
    else step = 12;                  // slow regen (was 6)

    if (diffSeconds >= step) {
      energy += 1;
      diffSeconds -= step;
      advancedMs += step * 1000;
      gained += 1;
    } else break;
  }
  ...
  // If we did not gain energy, keep cursor.
  // If we hit the per-call cap, advance cursor to now to avoid "banked time" being claimed via rapid API calls.
  const hitCap = gained >= MAX_GAIN_PER_CALL;
  const newLastTs = gained === 0 ? last : (hitCap ? now : new Date(advancedMs));
  ...
  await pool.query(
    ...
    WHERE id = $3
    `,
    [energy, newLastTs.toISOString(), user.id]
  );

  user.energy = energy;
  user.last_energy_ts = newLastTs;
...
app.post("/api/tap", async (req, res) => {
  try {
    ...
    if (currentEnergy <= 0) {
      const state = await buildClientState(user);
      return res.json({ ...state, ok: false, reason: "NO_ENERGY", seq: (req.body && req.body.seq) || null });
    }
    ...
    if (currentTaps >= maxTapsPerDay) {
      const state = await buildClientState(user);
      return res.json({ ...state, ok: false, reason: "MAX_TAPS_REACHED", seq: (req.body && req.body.seq) || null });
    }
    ...
    return res.json({ ...state, ok: true , seq: (req.body && req.body.seq) || null });
  } catch (err) {
...
app.post("/api/tapPacket", async (req, res) => {
  try {
    ...
    const n = Math.min(count, allowedByCap, allowedByEnergy);
    ...
    maybeActivateReferral(updatedUser, 'tap');
    const state = await buildClientState(updatedUser);
    return res.json({ ...state, ok: true, applied: n, gained , seq: (req.body && req.body.seq) || null });
  } catch (err) {
