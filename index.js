...
// ------------ Live event helper ------------
// Returns the currently active live event (if any) with a computed tapMultiplier.
// Results are lightly cached in-memory to avoid hammering the DB on every tap/state call.
let __liveEventCache = {
  value: null,
  expiresAt: 0,
};

async function getActiveLiveEvent() {
  const nowTs = Date.now();
  // 15s cache window is enough for real-time UX while keeping DB load low.
  if (__liveEventCache.expiresAt > nowTs) {
    return __liveEventCache.value;
  }

  try {
    const res = await pool.query(
      `
      SELECT id, code, title, starts_at, ends_at, config
    ...
    if (!res.rowCount) {
      __liveEventCache = { value: null, expiresAt: nowTs + 15_000 };
      return null;
    }
    ...
    try {
      ...
      console.warn("getActiveLiveEvent: failed to parse config JSON", e);
      cfg = {};
    }

    // Prefer an explicit tap multiplier; fall back to global_multiplier if present.
    let tapMultiplier = 1;
    const rawMult =
      cfg.tap_multiplier ??
      cfg.tapMultiplier ??
    ...
    __liveEventCache = { value: active, expiresAt: nowTs + 15_000 };
    return active;
  } catch (err) {
    console.error("getActiveLiveEvent failed:", err);
    __liveEventCache = { value: null, expiresAt: nowTs + 5_000 };
    return null;
  }
}
...
async function buildClientState(user) {
  ...
  const activeEvent = await getActiveLiveEvent();
  let liveEventForClient = null;
  if (activeEvent) {
    const startsAt =
      activeEvent.starts_at instanceof Date
        ? activeEvent.starts_at.toISOString()
        : (activeEvent.starts_at ? new Date(activeEvent.starts_at).toISOString() : null);
    const endsAt =
      activeEvent.ends_at instanceof Date
        ? activeEvent.ends_at.toISOString()
        : (activeEvent.ends_at ? new Date(activeEvent.ends_at).toISOString() : null);

    liveEventForClient = {
      code: activeEvent.code,
      title: activeEvent.title,
      tap_multiplier: Number(activeEvent.tapMultiplier || 1),
      starts_at: startsAt,
      ends_at: endsAt,
    };
  }
...
async function applyMissionReward(user, mission) {
  // Mission rewards are usually point-based. Historically some sponsor missions
  // used payout_type = 'mission' as a special bridge type, which resulted in a
  // no-op for user balance. This caused quests to show as claimed without
  // actually crediting points. To keep legacy data safe while ensuring sponsor
  // quests pay out, we normalise the reward type here.
  const rawType = mission && mission.payout_type ? String(mission.payout_type) : "points";
  const kind = mission && mission.kind ? String(mission.kind).toLowerCase() : "";
  ...
  // For sponsor missions we always want to credit the user with points,
  // even if older rows stored payout_type = 'mission'.
  if (kind === "sponsor" && effectiveType.toLowerCase() === "mission") {
    effectiveType = "points";
  }

  return applyGenericReward(user, effectiveType, mission.payout_amount);
}
...
app.post("/api/tap", async (req, res) => {
  try {
    ...
    // 5) Spend 1 energy + add points (double if boost still active + live event bonus)
    const basePerTap = 1;
    const now = new Date();
    ...
    // Apply live event multiplier on top of any boost (if configured)
    const activeEventForTap = await getActiveLiveEvent();
    if (activeEventForTap && activeEventForTap.tapMultiplier && activeEventForTap.tapMultiplier > 0) {
      perTap = perTap * activeEventForTap.tapMultiplier;
    }
...
app.post("/api/tapPacket", async (req, res) => {
  try {
    ...
    // Apply live event multiplier (if any) on top of tap boosts
    const activeEventForPacket = await getActiveLiveEvent();
    if (activeEventForPacket && activeEventForPacket.tapMultiplier && activeEventForPacket.tapMultiplier > 0) {
      perTap = perTap * activeEventForPacket.tapMultiplier;
    }
...
// Get active events (simple) – backed by live_events with configurable multipliers
app.post("/api/events/active", async (req, res) => {
  try {
    ...
    const now = new Date();
    const endsAt =
      active.ends_at instanceof Date
        ? active.ends_at
        : (active.ends_at ? new Date(active.ends_at) : null);
    const startsAt =
      active.starts_at instanceof Date
        ? active.starts_at
        : (active.starts_at ? new Date(active.starts_at) : null);

    let secondsLeft = null;
    if (endsAt && !Number.isNaN(endsAt.getTime())) {
      secondsLeft = Math.max(0, Math.round((endsAt.getTime() - now.getTime()) / 1000));
    }
    ...
    const eventPayload = {
      ...
      season_name: cfg.season_name || null,
      tap_multiplier: Number(active.tapMultiplier || 1),
      starts_at: startsAt ? startsAt.toISOString() : null,
      ends_at: endsAt ? endsAt.toISOString() : null,
      seconds_left: secondsLeft,
    };

    return res.json({ ok: true, events: [eventPayload] });
  } catch (err) {
...
const um = umRes.rows[0];
  ...
  try {
    ...
    // If the ledger path fails for any reason (missing table, bad migration,
    // or permissions), fall back to a direct balance update so the user
    // ALWAYS receives their sponsor mission reward.
    console.error("Sponsor mission payout via ledger failed, falling back to direct balance update:", err && err.message ? err.message : err);
    await client.query(
      `
      UPDATE public.users
      SET balance = balance + $1
