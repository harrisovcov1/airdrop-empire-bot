...
// Sponsor mission integrity: when enabled, sponsor claims require server-to-server verification
// (e.g., OGAds postback sets user_missions.verified_at). This prevents client-only “wait and claim” fraud.
const SPONSOR_REQUIRE_VERIFICATION = String(
  process.env.SPONSOR_REQUIRE_VERIFICATION || (OGADS_POSTBACK_SECRET ? "1" : "0")
).trim() === "1";

// Internal sponsor missions that are not tied to external offers/postbacks.
// These are allowed to complete without verification (still rate-limited + idempotent).
const INTERNAL_SPONSOR_CODES = new Set(["sp_double_points", "sp_emergency_energy", "sp_watch_video"]);
...
// REQUIRE_TELEGRAM_AUTH=1 will require valid Telegram initData signatures for API access.
// ALLOW_DEV_FALLBACK=1 allows telegram_id fallback (useful for local/dev only).
const REQUIRE_TELEGRAM_AUTH = String(process.env.REQUIRE_TELEGRAM_AUTH || "1") === "1";
const ALLOW_DEV_FALLBACK = String(process.env.ALLOW_DEV_FALLBACK || "0") === "1";
// Reject very old initData by default (seconds). Telegram sends auth_date.
const TELEGRAM_INITDATA_MAX_AGE_SEC = Number(process.env.TELEGRAM_INITDATA_MAX_AGE_SEC || 86400); // 24h
...
function respondApiError(res, err, fallbackCode = 500, fallbackError = "SERVER_ERROR") {
  const code = Number(err && (err.statusCode || err.status)) || fallbackCode;
  if (code === 401) {
  ...
  return res.status(code).json({ ok: false, error: fallbackError });
}
...
// ------------ Rate limiting ---------
// In-memory limiter kept as a fallback, but production uses DB-backed shared limiter (Fix #3)
const RATE = new Map();
...
async function hitShared(key, route, limit, windowMs) {
  // Allow turning off shared limiter if ever needed
  const useShared = (process.env.SHARED_RATE_LIMIT || "1") === "1";
  if (!useShared) return hitMemory(`${route}:${key}`, limit, windowMs);

  const windowSec = Math.max(1, Math.floor(windowMs / 1000));
  const intervalStr = `${windowSec} seconds`;

  const client = await pool.connect();
  try {
    const q = `
      INSERT INTO public.rate_limits (key, route, window_start, count)
      VALUES ($1, $2, now(), 1)
      ON CONFLICT (key, route) DO UPDATE
      SET
        window_start = CASE
          WHEN public.rate_limits.window_start < now() - $3::interval THEN now()
          ELSE public.rate_limits.window_start
        END,
        count = CASE
          WHEN public.rate_limits.window_start < now() - $3::interval THEN 1
          WHEN public.rate_limits.count < $4 THEN public.rate_limits.count + 1
      ...
      RETURNING
        ...
        count,
        CASE
          WHEN public.rate_limits.window_start < now() - $3::interval THEN TRUE
          WHEN public.rate_limits.count < $4 THEN TRUE
          ELSE FALSE
        END AS allowed;
    `;
    const r = await client.query(q, [String(key), String(route), intervalStr, Number(limit)]);
    const row = r.rows[0] || {};
    return {
      allowed: !!row.allowed,
...
function calcRetryAfter(windowStart, windowSec) {
  try {
    const ws = new Date(windowStart).getTime();
    const now = Date.now();
    const elapsed = Math.max(0, now - ws);
    const remaining = Math.max(0, windowSec * 1000 - elapsed);
...
async function ensureStripeFulfillmentTable() {
  try {
    await pool.query(`
      CREATE TABLE IF NOT EXISTS public.stripe_fulfilled_objects (
        object_id TEXT PRIMARY KEY,
        created_at TIMESTAMPTZ DEFAULT NOW()
      );
    `);
  } catch (e) {
    // If the DB user lacks permissions, we skip secondary dedupe.
  }
}

// Returns {first:true} if this object_id has not been fulfilled before.
async function markStripeObjectFulfilled(objectId) {
  if (!objectId) return { ok: true, first: true };
  await ensureStripeFulfillmentTable();
  try {
    const r = await pool.query(
      `INSERT INTO public.stripe_fulfilled_objects (object_id) VALUES ($1)
       ON CONFLICT (object_id) DO NOTHING
       RETURNING object_id;`,
      [String(objectId)]
    );
    return { ok: true, first: r.rowCount > 0 };
  } catch (e) {
...
// Periodic cleanup for shared rate limits table (keeps DB lean)
setInterval(async () => {
  try {
    const client = await pool.connect();
    try {
      await client.query(`DELETE FROM public.rate_limits WHERE window_start < now() - interval '7 days';`);
    } finally {
      client.release();
    }
  } catch (e) {}
...
// Trust proxy so req.ip etc behave correctly behind Render/Cloudflare/Proxies
app.set("trust proxy", 1);
app.disable("x-powered-by");

// ------------ Security headers (lightweight, safe for Telegram WebApp) ------------
app.use((req, res, next) => {
  res.setHeader("X-Content-Type-Options", "nosniff");
  res.setHeader("Referrer-Policy", "no-referrer");
  res.setHeader("X-DNS-Prefetch-Control", "off");
  res.setHeader("Permissions-Policy", "geolocation=(), microphone=(), camera=()");
  // Do NOT set X-Frame-Options DENY here (Telegram WebApp embedding/webview compatibility)
  next();
});
...
function buildAllowedOrigins() {
  ...
  const addExact = (v) => {
    if (!v) return;
    if (Array.isArray(v)) v.forEach(addExact);
    else exact.add(String(v).trim());
  };

  addExact(process.env.FRONTEND_ORIGIN);
  addExact(process.env.APP_BASE_URL);
  addExact(process.env.WEB_APP_URL);

  // Common Telegram origins (WebApp can be opened via these)
  addExact([
  ...
  // Dev convenience (only when explicitly allowed)
  const allowDev = String(process.env.ALLOW_DEV_FALLBACK || "0") === "1";
  if (allowDev || process.env.NODE_ENV !== "production") {
    addExact(["http://localhost:3000", "http://127.0.0.1:3000"]);
  }
...
const { exact: ALLOWED_ORIGINS, prefixes: ALLOWED_ORIGIN_PREFIXES } = buildAllowedOrigins();

function isAllowedOrigin(origin) {
  if (!origin) return true; // webhooks / server-to-server / curl
  if (ALLOWED_ORIGINS.has(origin)) return true;
  for (const p of ALLOWED_ORIGIN_PREFIXES) {
    if (origin.startsWith(p)) return true;
  }
  return false;
}
...
// Simple health endpoint (for Render health checks / uptime monitoring)
...
app.use(async (req, res, next) => {
  try {
    ...
    if (!skipGlobalReqLimit) {
      const rl = await hitShared(fp, "req", 120, 60_000);
      if (!rl.allowed) {
        return res.status(429).json({ ok: false, error: "RATE_LIMIT", retry_after: calcRetryAfter(rl.window_start, rl.windowSec) });
      }
    }
  } catch (e) {}
  next();
...
app.post("/api/shop/coin-packs", async (req, res) => {
  try {
    ...
    const rl = await hitShared(fp, "shop", 30, 60_000);
    if (!rl.allowed) {
      return res.status(429).json({ ok: false, error: "RATE_LIMIT", retry_after: calcRetryAfter(rl.window_start, rl.windowSec) });
    }
...
app.post("/api/vip/product", async (req, res) => {
  try {
    const fp = req._fp || getClientFingerprint(req);
    const rl = await hitShared(fp, "vipprod", 30, 60_000);
    if (!rl.allowed) {
      return res.status(429).json({ ok: false, error: "RATE_LIMIT", retry_after: calcRetryAfter(rl.window_start, rl.windowSec) });
    }
...
app.post("/api/payments/create-checkout", async (req, res) => {
  try {
    const fp = req._fp || getClientFingerprint(req);
    const rl = await hitShared(fp, "payco", 25, 60_000);
    if (!rl.allowed) {
      return res.status(429).json({ ok: false, error: "RATE_LIMIT", retry_after: calcRetryAfter(rl.window_start, rl.windowSec) });
    }
...
app.post("/api/vip/create-checkout", async (req, res) => {
  try {
    const fp = req._fp || getClientFingerprint(req);
    const rl = await hitShared(fp, "vipco", 20, 60_000);
    if (!rl.allowed) {
      return res.status(429).json({ ok: false, error: "RATE_LIMIT", retry_after: calcRetryAfter(rl.window_start, rl.windowSec) });
    }
...
app.post("/api/stripe/webhook", async (req, res) => {
  try {
    ...
    if (event.type === "checkout.session.completed") {
      ...
      const stripeObjectId = String(session.payment_intent || session.id || "").trim();
      try {
        const ded = await markStripeObjectFulfilled(stripeObjectId);
        if (ded.ok && !ded.first) {
          return res.status(200).send("ok");
        }
      } catch (e) {}
      ...
      if (telegramId) {
        ...
        if (coinsGranted && Number.isFinite(coinsGranted) && coinsGranted > 0) {
          await pool.query(
            ...
            [Math.floor(coinsGranted), telegramId]
          );
        }
...
async function ensureCoinbaseFulfillmentTable() {
  try {
    await pool.query(`
      CREATE TABLE IF NOT EXISTS public.coinbase_fulfilled_charges (
...
// Returns {first:true} if this charge_code has not been fulfilled before.
async function markCoinbaseChargeFulfilled(chargeCode) {
  if (!chargeCode) return { ok: true, first: true };
  await ensureCoinbaseFulfillmentTable();
  try {
    const r = await pool.query(
      `INSERT INTO public.coinbase_fulfilled_charges (charge_code) VALUES ($1)
...
app.post("/api/coinbase/webhook", async (req, res) => {
  try {
    ...
    if (type === "charge:confirmed" || type === "charge:resolved") {
      ...
      try {
        const ded = await markCoinbaseChargeFulfilled(providerPaymentId);
        if (ded.ok && !ded.first) {
          return res.status(200).send("ok");
        }
      } catch (e) {}
        ...
        if (coinsGranted && Number.isFinite(coinsGranted) && coinsGranted > 0 && providerPaymentId) {
          await pool.query(
            ...
            SET balance = u.balance + $1,
                updated_at = NOW()
            WHERE u.telegram_id = ($2)::bigint
              AND EXISTS (
                SELECT 1
                FROM public.purchase_ledger pl
                WHERE pl.provider = 'coinbase'
                  AND pl.provider_payment_id = $3
                  AND COALESCE(pl.coins_granted, 0) = $1
              );
            `,
            [Math.floor(coinsGranted), telegramIdRaw, providerPaymentId]
          );
        }
      }
...
// Telegram WebApp initData verification (prevents telegram_id spoofing)
...
function verifyTelegramInitData(initDataRaw) {
  try {
    if (!initDataRaw) return { ok: false, error: "missing" };
    const params = new URLSearchParams(initDataRaw);
    ...
    const secretKey = crypto.createHmac("sha256", "WebAppData").update(String(BOT_TOKEN)).digest();
    const computed = crypto.createHmac("sha256", secretKey).update(dataCheckString).digest("hex");

    const ok = computed.toLowerCase() === String(hash).toLowerCase();
    if (!ok) return { ok: false, error: "bad_hash" };

    // Freshness (optional but recommended)
    const authDate = Number(params.get("auth_date") || 0);
    if (authDate && Number.isFinite(authDate) && TELEGRAM_INITDATA_MAX_AGE_SEC > 0) {
      const nowSec = Math.floor(Date.now() / 1000);
      if (nowSec - authDate > TELEGRAM_INITDATA_MAX_AGE_SEC) {
...
/**
 * Compute a soft risk score (0..100+) for anti-farm / anti-multi-account.
 * This does NOT block normal play; we only gate high-value actions (withdrawals / external sponsor payouts).
 */
async function computeRiskScore(client, userId, primaryFingerprint, ipHash) {
  let score = 0;
  try {
    // Same fingerprint used by multiple users recently
    if (primaryFingerprint) {
      const r1 = await client.query(
        `SELECT COUNT(DISTINCT user_id) AS c
         FROM public.user_devices
         WHERE fingerprint = $1 AND last_seen > NOW() - interval '24 hours'`,
        [primaryFingerprint]
      );
      const c = Number(r1.rows?.[0]?.c || 0);
      if (c >= 3) score += 70;
      else if (c === 2) score += 30;
    }

    // Same IP hash used by many users recently (proxy farms)
    if (ipHash) {
      const r2 = await client.query(
        `SELECT COUNT(DISTINCT user_id) AS c
         FROM public.user_devices
         WHERE last_ip_hash = $1 AND last_seen > NOW() - interval '24 hours'`,
        [ipHash]
      );
      const c2 = Number(r2.rows?.[0]?.c || 0);
      if (c2 >= 8) score += 60;
      else if (c2 >= 5) score += 40;
      else if (c2 >= 3) score += 20;
    }

    // One user rotating too many fingerprints (spoofing)
    ...
    const c3 = Number(r3.rows?.[0]?.c || 0);
    if (c3 >= 10) score += 40;
    else if (c3 >= 6) score += 20;
...
// Risk / device integrity fields (soft anti-fraud)
await client.query(`
  ALTER TABLE public.users
    ...
    ADD COLUMN IF NOT EXISTS last_ua_hash TEXT;
`);
...
`);
await client.query(`
  CREATE INDEX IF NOT EXISTS idx_user_devices_fingerprint_lastseen
    ON public.user_devices (fingerprint, last_seen DESC);
`);
...
`);
  ...
  // Global idempotency keys for spend/award endpoints (prevents double-click + retry double-credit).
  await client.query(`
  ...
  `);
  await client.query(`CREATE INDEX IF NOT EXISTS idx_rate_limits_window_start ON public.rate_limits (window_start);`);




// Indexes / constraints for performance + stability
  await client.query(`
    -- Users
    ...
    CREATE INDEX IF NOT EXISTS idx_users_vip_until ON public.users (vip_until DESC);
    CREATE INDEX IF NOT EXISTS idx_users_double_until ON public.users (double_boost_until DESC);

    -- Referrals
    CREATE INDEX IF NOT EXISTS idx_referrals_inviter ON public.referrals (inviter_id);
    CREATE INDEX IF NOT EXISTS idx_referrals_invited ON public.referrals (invited_id);
    ...
    -- Withdrawals
    CREATE INDEX IF NOT EXISTS idx_withdraw_user_created ON public.withdraw_requests (user_id, created_at DESC);
    CREATE INDEX IF NOT EXISTS idx_withdraw_status_created ON public.withdraw_requests (status, created_at DESC);

    -- Anti-fraud devices
    CREATE INDEX IF NOT EXISTS idx_user_devices_user_lastseen ON public.user_devices (user_id, last_seen DESC);

    -- Event log
    CREATE INDEX IF NOT EXISTS idx_event_log_user_created ON public.event_log (user_id, created_at DESC);

    -- Tap packet idempotency
    CREATE INDEX IF NOT EXISTS idx_tap_packets_telegram_created ON public.tap_packets (telegram_id, created_at DESC);

    -- Global idempotency table
    CREATE UNIQUE INDEX IF NOT EXISTS idempotency_keys_unique
      ON public.idempotency_keys (user_id, endpoint, request_id);
    CREATE INDEX IF NOT EXISTS idx_idempotency_user_created
      ON public.idempotency_keys (user_id, created_at DESC);
  ...
  // Points ledger (industrial-grade audit trail)
  await client.query(`
    ...
    CREATE INDEX IF NOT EXISTS idx_points_ledger_tg_created
      ON public.points_ledger (telegram_id, created_at DESC);

    CREATE INDEX IF NOT EXISTS idx_points_ledger_reason_created
      ON public.points_ledger (reason, created_at DESC);

    CREATE INDEX IF NOT EXISTS idx_points_ledger_endpoint_created
      ON public.points_ledger (endpoint, created_at DESC);
  `);
}
// ---------------------------
// Idempotency helpers
...
function getRequestId(req) {
  // Client can send request_id in JSON body, querystring, or X-Request-Id header.
  const h = req.headers && (req.headers["x-request-id"] || req.headers["x-requestid"]);
  return (
    (req.body && (req.body.request_id || req.body.requestId)) ||
    (req.query && (req.query.request_id || req.query.requestId)) ||
    h ||
    null
  );
}

async function idempotencyLookup(client, userId, endpoint, requestId) {
  if (!requestId) return { hit: false };

  const r = await client.query(
    `SELECT status, response
     FROM public.idempotency_keys
     WHERE user_id = $1 AND endpoint = $2 AND request_id = $3
     LIMIT 1`,
    [userId, endpoint, requestId]
  );

  if (r.rowCount === 0) return { hit: false };

  const row = r.rows[0];
  if (row.status === "done" && row.response) return { hit: true, response: row.response };
  if (row.status === "pending") return { hit: true, pending: true };
  if (row.status === "error" && row.response) return { hit: true, response: row.response };
  return { hit: true, pending: true };
}

async function idempotencyBegin(client, userId, endpoint, requestId) {
  if (!requestId) return { ok: true, inserted: false };
  try {
    await client.query(
      `INSERT INTO public.idempotency_keys (user_id, endpoint, request_id, status)
    ...
    // If table doesn't exist (shouldn't happen if AUTO_MIGRATE), treat as non-idempotent.
    return { ok: false, inserted: false };
  }
}

async function idempotencyFinish(client, userId, endpoint, requestId, responseObj, status = "done") {
  if (!requestId) return;
  await client.query(
    `UPDATE public.idempotency_keys
     SET status = $4, response = $5, updated_at = now()
     WHERE user_id = $1 AND endpoint = $2 AND request_id = $3`,
    [userId, endpoint, requestId, status, responseObj]
  );
}
...
async function getOrCreateUserFromInitData(req) {
  // Telegram may send initData via body/query; some clients may pass it via header.
  const initDataRaw = String(req.headers["x-telegram-initdata"] || req.body.initData || req.query.initData || "");
  const data = parseInitData(initDataRaw);

  // Enforce Telegram initData signature verification (prevents spoofing telegram_id)
  if (initDataRaw) {
    const v = verifyTelegramInitData(initDataRaw);
    if (!v.ok && REQUIRE_TELEGRAM_AUTH) {
      const err = new Error(`Invalid Telegram initData (${v.error})`);
      err.statusCode = 401;
      throw err;
    }
  } else {
    if (REQUIRE_TELEGRAM_AUTH && !ALLOW_DEV_FALLBACK) {
      const err = new Error("Missing Telegram initData");
      err.statusCode = 401;
      throw err;
    }
  }
  ...
  if (!telegramUserId) {
    if (REQUIRE_TELEGRAM_AUTH && !ALLOW_DEV_FALLBACK) {
      const err = new Error("Telegram initData required (dev fallback disabled)");
      err.statusCode = 401;
      throw err;
    }
  ...
  try {
    ...
    if (inviterId && inviterId && inviterId !== telegramUserId) {
      ...
      const refRes = await client.query(
        `SELECT 1 FROM public.referrals WHERE inviter_id = $1 AND invited_id = $2 LIMIT 1;`,
        [inviterId, telegramUserId]
      );

      if (refRes.rowCount === 0) {
        await client.query(
          `INSERT INTO public.referrals (inviter_id, invited_id) VALUES ($1, $2);`,
          [inviterId, telegramUserId]
        );
...
// device fingerprint + soft risk scoring
try {
  const fp = getClientFingerprint(req);
  const meta = getClientMeta(req);

  // Store hashed IP/UA to avoid raw PII retention
  await client.query(
    `INSERT INTO public.user_devices (user_id, fingerprint, last_ip_hash, last_ua_hash)
     VALUES ($1,$2,$3,$4)
     ON CONFLICT (user_id, fingerprint)
     DO UPDATE SET last_seen = NOW(),
                  last_ip_hash = EXCLUDED.last_ip_hash,
                  last_ua_hash = EXCLUDED.last_ua_hash`,
    [user.id, fp, meta.ip_hash, meta.ua_hash]
  );
    ...
    await client.query(
      `INSERT INTO public.user_devices (user_id, fingerprint, last_ip_hash, last_ua_hash)
       ...
       DO UPDATE SET last_seen = NOW(),
                    last_ip_hash = EXCLUDED.last_ip_hash,
                    last_ua_hash = EXCLUDED.last_ua_hash`,
      [user.id, cidFp, meta.ip_hash, meta.ua_hash]
    );
  }

  // Update user meta hashes (for support + anti-abuse)
  await client.query(
    `UPDATE public.users
     SET last_ip_hash = COALESCE($2, last_ip_hash),
         last_ua_hash = COALESCE($3, last_ua_hash)
     WHERE id = $1`,
    [user.id, meta.ip_hash, meta.ua_hash]
  );

  // Compute risk score and flag (soft-gate only high-value actions)
  const riskScore = await computeRiskScore(client, user.id, fp, meta.ip_hash);
  const riskFlag = riskScore >= 80;
  await client.query(
    `UPDATE public.users SET risk_score = $2, risk_flag = $3 WHERE id = $1`,
    [user.id, riskScore, riskFlag]
  );

  // Keep response user object in sync
  user.risk_score = riskScore;
  user.risk_flag = riskFlag;
  user.last_ip_hash = meta.ip_hash || user.last_ip_hash;
  user.last_ua_hash = meta.ua_hash || user.last_ua_hash;
...
async function maybeAwardEarlyAccessStarterBonus(client, user) {
  ...
  try {
    ...
    await client.query("UPDATE public.users SET balance = balance + $2 WHERE id = $1", [userId, BONUS]);

    const afterRes = await client.query("SELECT balance FROM public.users WHERE id = $1", [userId]);
    const after = Number(afterRes.rows?.[0]?.balance ?? before + BONUS);
...
async function recordPointsLedger({
  client = pool,
  user_id,
  telegram_id,
  delta,
  ...
  try {
    await client.query(
      `INSERT INTO public.points_ledger
        (user_id, telegram_id, delta, balance_before, balance_after, reason, endpoint, request_id, meta)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9::jsonb)`,
      [
        Number(user_id),
        telegram_id != null ? Number(telegram_id) : null,
        Number(delta || 0),
        balance_before != null ? Number(balance_before) : null,
        balance_after != null ? Number(balance_after) : null,
    ...
    // Never break core flows due to logging
  }
}
...
async function handleOgadsPostback(req, res) {
  try {
    ...
    if (!um.reward_applied) {
      ...
      await logEvent(userId, "ogads_conversion_credited", { transaction_id: transactionId, mission: mission.code, offer_id: offerId, payout });
...
app.get("/api/admin/export/ledger.csv", async (req, res) => {
  try {
    const key = String(req.headers["x-admin-key"] || "");
    if (!ADMIN_KEY || key !== ADMIN_KEY) return res.status(403).send("FORBIDDEN");
    const q = await pool.query(`SELECT * FROM public.points_ledger ORDER BY id DESC LIMIT 50000`);
    res.setHeader("content-type","text/csv; charset=utf-8");
    res.send(rowsToCsv(q.rows));
  } catch (e) { res.status(500).send("ERROR"); }
});

app.get("/api/admin/ledger/recent", async (req, res) => {
  try {
    const key = String(req.headers["x-admin-key"] || "");
    if (!ADMIN_KEY || key !== ADMIN_KEY) return res.status(403).json({ ok:false, error:"FORBIDDEN" });
    const limit = Math.max(1, Math.min(1000, Number(req.query.limit || 200)));
    const q = await pool.query(`SELECT * FROM public.points_ledger ORDER BY id DESC LIMIT $1`, [limit]);
    res.json({ ok:true, rows: q.rows });
  } catch (e) { res.status(500).json({ ok:false, error:"ERROR" }); }
});
...
app.post("/api/state", async (req, res) => {
  try {
    ...
    return respondApiError(res, err, 500, "STATE_ERROR");
  }
});
...
app.post("/api/tap", async (req, res) => {
  try {
    ...
    const fp = req._fp || getClientFingerprint(req);
    if (!hitMemory(`tapburst:${fp}`, 25, 1000)) {
    ...
    // Ledger (audit trail)
    const __deltaTap = Number(updatedUser.balance || 0) - Number(user.balance || 0);
    if (__deltaTap !== 0) {
      await recordPointsLedger({
        user_id: updatedUser.id,
        telegram_id: updatedUser.telegram_id,
        delta: __deltaTap,
        balance_before: Number(user.balance || 0),
        balance_after: Number(updatedUser.balance || 0),
        reason: "tap",
        endpoint: "/api/tap",
        request_id: null,
        meta: { delta: __deltaTap },
      });
    }

    // Ledger (audit trail)
    if (Number(gained || 0) !== 0) {
      await recordPointsLedger({
        user_id: updatedUser.id,
        telegram_id: updatedUser.telegram_id,
        ...
        balance_after: Number(updatedUser.balance || 0),
        reason: "tap_packet",
        endpoint: "/api/tapPacket",
        request_id: packetId || null,
        meta: { applied: n, gained: Number(gained || 0) },
      });
    }
...
app.post("/api/tapPacket", async (req, res) => {
  try {
    ...
    const fp = req._fp || getClientFingerprint(req);
    if (!hitMemory(`tapburst:${fp}`, 12, 1000)) {
      return res.status(429).json({ ok: false, error: "TAP_RATE_LIMIT" });
    }
...
app.post("/api/boost/energy", async (req, res) => {
  try {
    let user = await getOrCreateUserFromInitData(req);
    const __endpoint = "/api/boost/energy";
    const requestId = getRequestId(req);
    if (requestId) {
      const existing = await idempotencyLookup(pool, user.id, __endpoint, requestId);
      if (existing && existing.hit) {
        if (existing.pending) return res.status(409).json({ ok: false, reason: "IN_PROGRESS" });
        return await send(existing.response);
      }
      await idempotencyBegin(pool, user.id, __endpoint, requestId);
    }

    const send = async (obj) => {
      try {
        if (requestId) await idempotencyFinish(pool, user.id, __endpoint, requestId, obj, obj && obj.ok === false ? "error" : "done");
      } catch (e) {}
    ...
    if (currentEnergy >= maxEnergy) {
      const state = await buildClientState(user);
      return await send({ ...state, ok: false, reason: "ENERGY_FULL" });
    }
    ...
    if (method === "points") {
      ...
      // Ledger (audit trail): spent points for energy refill
      await recordPointsLedger({
        user_id: updatedUser.id,
        telegram_id: updatedUser.telegram_id,
        delta: -Number(ENERGY_REFILL_COST || 0),
        balance_before: Number(user.balance || 0),
        balance_after: Number(updatedUser.balance || 0),
        reason: "boost_energy_spend",
        endpoint: "/api/boost/energy",
        request_id: requestId || null,
        meta: { cost: Number(ENERGY_REFILL_COST || 0) },
      });
...
app.post("/api/boost/double", async (req, res) => {
  try {
    let user = await getOrCreateUserFromInitData(req);
    const __endpoint = "/api/boost/double";
    const requestId = getRequestId(req);
    if (requestId) {
      const existing = await idempotencyLookup(pool, user.id, __endpoint, requestId);
      if (existing && existing.hit) {
    ...
    if (method === "points") {
      ...
      if (currentBalance < DOUBLE_BOOST_COST) {
        const state = await buildClientState(user);
        return await send({ ...state, ok: false, reason: "NOT_ENOUGH_POINTS" });
      }
      ...
      updatedUser = upd.rows[0];
      // Ledger (audit trail): spent points for double boost
      await recordPointsLedger({
        user_id: updatedUser.id,
        telegram_id: updatedUser.telegram_id,
        delta: -Number(DOUBLE_BOOST_COST || 0),
        balance_before: Number(user.balance || 0),
        balance_after: Number(updatedUser.balance || 0),
        reason: "boost_double_spend",
        endpoint: "/api/boost/double",
        request_id: requestId || null,
        meta: { cost: Number(DOUBLE_BOOST_COST || 0) },
      });
    ...
    if (method === "points") {
      ...
      const upd = await pool.query(
        ...
        SET balance = balance - $1,
            ...
            max_energy = GREATEST(COALESCE(max_energy, 50), $3),
            energy = LEAST(GREATEST(COALESCE(energy, 0), 0), GREATEST(COALESCE(max_energy, 50), $3))
        WHERE id = $4
        RETURNING *;
        `,
        [cost, newUntil.toISOString(), VIP_MAX_ENERGY, user.id]
      );
      user = upd.rows[0];
...
app.post("/api/mission/complete", async (req, res) => {
  try {
    let user = await getOrCreateUserFromInitData(req);
    const __endpoint = "/api/mission/complete";
    const requestId = getRequestId(req);

    const send = async (obj, status = 200) => {
      try {
        if (requestId) {
          await idempotencyFinish(
            pool,
            user.id,
            __endpoint,
            requestId,
            obj,
            obj && obj.ok === false ? "error" : "done"
          );
        }
      } catch (e) {}
...
// Soft anti-fraud gate: block external sponsor payouts for flagged accounts (normal play unaffected)
const __internalSponsor = new Set(["sp_double_points", "sp_emergency_energy", "sp_watch_video"]);
if (user.risk_flag && (mission.kind === "sponsor") && !__internalSponsor.has(code)) {
  return await send({ ok: false, error: "RISK_REVIEW" }, 403);
}
    ...
    const isSponsorForStartGate = (String(mission.kind || "").toLowerCase() === "sponsor") || String(mission.code || "").startsWith("sp_");

    if (!umRes.rows.length || !umRes.rows[0].started_at) {
      if (!isSponsorForStartGate) {
        return await send({ ...(await buildClientState(user)), ok: false, error: "MISSION_NOT_STARTED" });
      }
    ...
    const isSponsorMission = (String(mission.kind || "").toLowerCase() === "sponsor") || String(mission.code || "").startsWith("sp_");
    ...
    if (isSponsorMission) {
      ...
      try {
        ...
        const mRes = await client.query(
          `SELECT id, code, payout_amount, min_seconds_to_claim, cooldown_hours, max_claims_per_day, is_active
           FROM public.missions
        ...
        const umFind = await client.query(
          `SELECT id, started_at, claimed_at, verified_at, reward_applied
           FROM public.user_missions
           WHERE user_id = $1 AND mission_id = $2
           ORDER BY id DESC
           LIMIT 1`,
        ...
        if (umRow.reward_applied) {
          await client.query("ROLLBACK");
          // Already fulfilled (often via OGAds postback). Return authoritative state.
          const uRes = await pool.query(`SELECT * FROM public.users WHERE id = $1 LIMIT 1`, [userId]);
          const state = await buildClientState(uRes.rows && uRes.rows[0] ? uRes.rows[0] : user);
          return await send({
            ...state,
            ok: true,
            already_completed: true,
            mission: { code: mRow.code || code, status: "completed", reward_applied: true },
          });
        }

        // Cooldown gate (server-authoritative)
        if (umRow.claimed_at) {
          const cooldownMs = Number(mRow.cooldown_hours || 24) * 60 * 60 * 1000;
          const claimedAt = new Date(umRow.claimed_at);
          const readyAt = new Date(claimedAt.getTime() + cooldownMs);
          if (readyAt > new Date()) {
            const remaining = Math.max(0, Math.floor((readyAt - new Date()) / 1000));
            await client.query("ROLLBACK");
            return await send({ ok: false, error: "MISSION_COOLDOWN", cooldown_remaining_seconds: remaining }, 400);
          }
        }

        // Max claims per day gate
        const maxPerDay = Number(mRow.max_claims_per_day || 1);
        if (maxPerDay > 0) {
          const cRes = await client.query(
            `SELECT COUNT(*)::int AS c
             FROM public.user_missions
             WHERE user_id=$1 AND mission_id=$2 AND claimed_at IS NOT NULL AND claimed_at::date = CURRENT_DATE`,
            [userId, mRow.id]
          );
          const c = Number(cRes.rows?.[0]?.c || 0);
          if (c >= maxPerDay) {
            await client.query("ROLLBACK");
            return await send({ ok: false, error: "MAX_CLAIMS_REACHED" }, 400);
          }
        }
        ...
        if (elapsed < minSec) {
          await client.query("ROLLBACK");
          return await send({ ok: false, error: "TOO_EARLY", remaining_seconds: (minSec - elapsed) }, 400);
        }

        // Verification hardening (server-to-server). Default enabled when OGADS_POSTBACK_SECRET is set.
        // Non-boost sponsor missions require verified_at to be set by a postback.
        const requireVerify = SPONSOR_REQUIRE_VERIFICATION && !INTERNAL_SPONSOR_CODES.has(String(mRow.code || code));
        if (requireVerify && !umRow.verified_at) {
          await client.query("ROLLBACK");
          return await send({ ok: false, error: "SPONSOR_NOT_VERIFIED_YET" }, 428);
        }

        const payout = Number(mRow.payout_amount || 0);
        ...
        await client.query(
          `UPDATE public.user_missions
           SET status = 'completed',
               ...
               claimed_at = now(),
               reward_applied = true,
               verified_at = COALESCE(verified_at, CASE WHEN $2 THEN now() ELSE verified_at END),
               updated_at = now()
           WHERE id = $1`,
          [umRow.id, INTERNAL_SPONSOR_CODES.has(String(mRow.code || code))]
        );
        ...
        await client.query("COMMIT");
        // Ledger (audit trail): sponsor mission payout (exactly-once via mission claim flow)
        if (payout > 0) {
          await recordPointsLedger({
            user_id: userId,
            ...
            balance_before: null,
            balance_after: null,
            reason: "sponsor_mission_payout",
            endpoint: "/api/mission/complete",
            request_id: requestId || null,
            meta: { mission_code: String(mRow.code || code), payout },
          });
        }


        // Return authoritative state so the client can reflect the credit instantly
        // without depending on a follow-up /api/state (which can be delayed by Telegram lifecycle/network).
        const uRes = await pool.query(`SELECT * FROM public.users WHERE id = $1 LIMIT 1`, [userId]);
        const freshUser = uRes.rows && uRes.rows[0] ? uRes.rows[0] : user;
        ...
        return await send({
          ...
          mission: { code: mRow.code || code, status: "completed", reward_applied: true },
          cooldown_seconds: Number(mRow.cooldown_hours || 24) * 60 * 60,
        });
      } catch (e) {
    ...
    // Non-sponsor missions: apply reward once, then mark mission completed.
    if (!um.reward_applied) {
      user = await applyMissionReward(user, mission);
...
app.post("/api/boost/completeAd", async (req, res) => {
  try {
    ...
    const fp = req._fp || getClientFingerprint(req);
    const rl = await hitShared(fp, "adcomplete", 10, 60 * 60 * 1000);
    if (!rl.allowed) {
      return res.status(429).json({ ok: false, error: "RATE_LIMIT", retry_after: calcRetryAfter(rl.window_start, rl.windowSec) });
    }
...
app.post("/api/task", async (req, res) => {
  try {
    ...
    if (!isDaily) {
      return await send({ ok: false, reason: "UNKNOWN_TASK" });
    }
    ...
    const newBalance = Number(user.balance || 0) + totalReward;
    ...
    const upd = await pool.query(
      ...
      RETURNING *;
      `,
      [newBalance, today, nowIso, newStreak, user.id]
    );

    user = upd.rows[0];
    // Ledger (audit trail): daily task reward
    if (Number(totalReward || 0) !== 0) {
      await recordPointsLedger({
        user_id: user.id,
        telegram_id: user.telegram_id,
        delta: Number(totalReward || 0),
        balance_before: Number(newBalance) - Number(totalReward || 0),
        balance_after: Number(user.balance || newBalance),
        reason: "daily_task",
        endpoint: "/api/task",
        request_id: requestId || null,
        meta: { streak: newStreak, reward: Number(totalReward || 0), bonus: Number(bonus || 0) },
      });
    }
...
if (user.risk_flag) {
  return res.status(403).json({ ok: false, error: "RISK_REVIEW" });
}

    const __endpoint = "/api/withdraw/request";
    const requestId = getRequestId(req);
    if (requestId) {
      const existing = await idempotencyLookup(pool, user.id, __endpoint, requestId);
      if (existing && existing.hit) {
    ...
    // Reserve funds immediately (prevents drain)
    await pool.query(
    ...
    res.json({ ok: true, request: wr.rows[0] });
  } catch (e) {
...
app.post("/api/admin/withdraw/update", async (req, res) => {
  try {
    ...
    if (status === "rejected") {
      ...
      await pool.query(`UPDATE public.users SET balance = balance + $1 WHERE id=$2`, [wr.amount, wr.user_id]);
    }
