...
// ------------ Live events helper ------------
// Simple cached lookup of the currently active live event.
// Driven by public.live_events so we can time-box multipliers from the DB.
let __liveEventCache = { value: null, expiresAt: 0 };

async function getActiveLiveEvent() {
  const nowMs = Date.now();
  if (__liveEventCache.expiresAt > nowMs) {
  ...
  try {
    ...
    if (!res.rowCount) {
      __liveEventCache = { value: null, expiresAt: nowMs + 15000 };
      return null;
    }
    ...
    __liveEventCache = { value: active, expiresAt: nowMs + 15000 };
    return active;
  } catch (err) {
    console.error("getActiveLiveEvent failed:", err);
    __liveEventCache = { value: null, expiresAt: nowMs + 5000 };
    return null;
  }
}
...
async function buildClientState(user) {
  ...
  if (activeEvent) {
    ...
    if (activeEvent.starts_at) {
      const d = activeEvent.starts_at instanceof Date ? activeEvent.starts_at : new Date(activeEvent.starts_at);
      if (!isNaN(d)) startsAt = d.toISOString();
    }
    if (activeEvent.ends_at) {
      const d2 = activeEvent.ends_at instanceof Date ? activeEvent.ends_at : new Date(activeEvent.ends_at);
      if (!isNaN(d2)) endsAt = d2.toISOString();
    }
...
app.post("/api/tap", async (req, res) => {
  try {
    ...
    // Apply live event tap multiplier on top of any boost (if configured)
...
app.post("/api/tapPacket", async (req, res) => {
  try {
    ...
    // Apply live event tap multiplier (if any) on top of tap boosts
