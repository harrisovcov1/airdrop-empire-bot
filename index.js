  ...
  try {
    ...
    const missionsRes = await pool.query(
      ...
      SELECT id, code, title, description, payout_type, payout_amount, url, kind,
             ...
             COALESCE(cooldown_hours, CASE WHEN kind = 'sponsor' THEN 1 ELSE 24 END) AS cooldown_hours,
...
app.post("/api/mission/complete", async (req, res) => {
  try {
    ...
    // Apply reward once.
const isPostbackVerifiedSponsor = String(mission.code || "").startsWith("sp_");
if (isPostbackVerifiedSponsor) {
