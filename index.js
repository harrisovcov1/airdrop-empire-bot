...
const STRICT_REFERRAL_ACTIVATION = String(process.env.STRICT_REFERRAL_ACTIVATION || "0") === "1";
const ONBOARDING_MIN_REFERRALS = Number(process.env.ONBOARDING_MIN_REFERRALS || 1);
...
// ------------ App config (feature flags, tuning) ------------
// Frontend uses this to read app_settings so we can tune behaviour without redeploying.
...
// Starter Bundle offer lookup
app.get("/api/shop/starter-offer", async (req, res) => {
  try {
    ...
    const starterSku = process.env.STARTER_PACK_SKU || "STARTER_1";

    // Look up user and ensure they are a normal player
    const { rows: userRows } = await pool.query(
      `
      select id, created_at, is_system_user, starter_offer_purchased
      from public.users
    ...
    if (userRows.length === 0 || userRows[0].is_system_user) {
      return res.json({
        success: true,
        available: false,
        reason: "not_eligible",
    ...
    // Product definition – support both Coinbase and Stripe Starter Bundle entries
    const { rows: productRows } = await pool.query(
      `
      select provider, pack_sku, coins_granted, currency, amount_minor, checkout_url
      from public.payment_products
      where pack_sku = $1
        and active = true
        and provider in ('coinbase', 'stripe')
      `,
      [starterSku],
    );

    const coinbaseProduct = productRows.find((r) => r.provider === "coinbase");
    const stripeProduct = productRows.find((r) => r.provider === "stripe");

    if ((!coinbaseProduct || !coinbaseProduct.checkout_url) && (!stripeProduct || !stripeProduct.checkout_url)) {
      return res.json({
        success: true,
        available: false,
        reason: "no_product",
    ...
    // Have they already purchased this starter pack?
    const { rows: existingRows } = await pool.query(
      ...
      limit 1
      `,
      [String(telegramIdRaw), starterSku],
    );

    const userRow = userRows[0];
    const alreadyPurchasedFlag = userRow && userRow.starter_offer_purchased === true;
    const alreadyPurchased = alreadyPurchasedFlag || existingRows.length > 0;

    const product = coinbaseProduct || stripeProduct;
    const currency = (product && product.currency) || "GBP";
    const amountMinor = (product && product.amount_minor) || 0;
    const coinsGranted = (product && product.coins_granted) || 0;

    return res.json({
      success: true,
      available: !alreadyPurchased,
      alreadyPurchased,
      telegramId,
      starterSku,
      ...
      currency,
      coinsGranted,
      checkoutUrl: coinbaseProduct ? coinbaseProduct.checkout_url : null,
      coinbaseCheckoutUrl: coinbaseProduct ? coinbaseProduct.checkout_url : null,
      stripeCheckoutUrl: stripeProduct ? stripeProduct.checkout_url : null,
    });
  } catch (err) {
    console.error("Error fetching starter offer", err);
...
app.post("/api/stripe/webhook", async (req, res) => {
  try {
    ...
    if (event.type === "checkout.session.completed") {
      ...
      let effectiveSku = normalizeSku(sku);
      let effectiveCoinsGranted = coinsGranted;

      // Map known Payment Links (like the Starter Bundle) to a SKU and coin amount when metadata is not present.
      try {
        const starterLinkId = process.env.STARTER_STRIPE_PAYMENT_LINK_ID || "plink_15m7OmJNksmNPDCyYQxFB3eH";
        if (paymentLinkId && starterLinkId && paymentLinkId === starterLinkId) {
          if (!effectiveSku || !String(effectiveSku).length) {
            effectiveSku = "starter_1";
          }
          if (!effectiveCoinsGranted || !Number.isFinite(effectiveCoinsGranted) || effectiveCoinsGranted <= 0) {
            effectiveCoinsGranted = 25000;
          }
        }
      } catch (e) {
        console.warn("Failed to normalize Stripe payment link SKU/coins", e && e.message ? e.message : e);
      }
      ...
      if (telegramId) {
        ...
        if (effectiveCoinsGranted && Number.isFinite(effectiveCoinsGranted) && effectiveCoinsGranted > 0) {
          const delta = Math.floor(effectiveCoinsGranted);
          try {
          ...
          // Mark Starter Bundle as purchased when the Stripe SKU matches.
          try {
            const starterSku = (process.env.STARTER_PACK_SKU || "STARTER_1").toUpperCase();
            if (effectiveSku && typeof effectiveSku === "string" && effectiveSku.toUpperCase() === starterSku && telegramId) {
              await pool.query(
                "UPDATE public.users SET starter_offer_purchased = TRUE WHERE telegram_id = $1::bigint;",
                [telegramId]
              );
            }
          } catch (e) {
            console.warn("Failed to mark starter_offer_purchased from Stripe webhook", e && e.message ? e.message : e);
          }
        }

        // 2) VIP (extend vip_until)
        if ((vipDays && Number.isFinite(vipDays) && vipDays > 0) || (effectiveSku && String(effectiveSku).toLowerCase() === "vip_30d")) {
      ...
      if (amountMinor != null) {
        ...
        if (coinsGranted && Number.isFinite(coinsGranted) && coinsGranted > 0 && providerPaymentId) {
          ...
          await withTransaction(async (client) => {
            ...
            // If this Coinbase purchase is the Starter Bundle, mark the starter offer as purchased.
            try {
              const starterSku = (process.env.STARTER_PACK_SKU || "STARTER_1").toUpperCase();
              if (packSku && typeof packSku === "string" && packSku.toUpperCase() === starterSku) {
                await client.query(
                  `
                  UPDATE public.users
                  SET starter_offer_purchased = TRUE
                  WHERE id = $1;
                  `,
                  [userId]
                );
              ...
              console.warn("Failed to mark starter_offer_purchased from Coinbase webhook", e && e.message ? e.message : e);
            }
...
async function ensureSchema(client) {
  ...
  // App-level settings / configuration (key -> JSON value)
  await client.query(`
...
// Run schema migrations once per process to avoid deadlocks under load.
...
async function ensureSchemaOnce() {
  ...
  schemaInitPromise = (async () => {
    ...
    try {
      ...
      console.error("ensureSchemaOnce failed:", err);
      throw err;
    } finally {
      client.release();
    }
...
async function getOrCreateUserFromInitData(req) {
  ...
  try {
    ...
    if (inviterId && inviterId && inviterId !== telegramUserId) {
      ...
      // Only act on the very first successful insert for this inviter+invited pair
      if (insRef.rowCount > 0) {
        // Look up inviter's internal user id + current referral stats
        ...
        if (inviterRes.rowCount > 0) {
          ...
          // Persist the referrer on the invited user (first writer wins)
          try {
            await client.query(
              `
              UPDATE public.users
          ...
          // Optionally reward immediately on join (default behaviour)
          if (!STRICT_REFERRAL_ACTIVATION) {
            ...
            // 2) Keep referral counters in sync (no direct balance mutation here)
            await client.query(
              `
              UPDATE public.users
...
async function buildClientState(user) {
  ...
  // Starter Bundle offer visibility (purely backend-driven)
  let starterOfferAvailable = false;
  let starterOfferExpiresAt = null;

  if (user.starter_offer_expires_at) {
    const starterExpires = new Date(user.starter_offer_expires_at);
    if (!isNaN(starterExpires)) {
      starterOfferExpiresAt = starterExpires.toISOString();
      if (!user.starter_offer_purchased && starterExpires > new Date()) {
  ...
  return {
    ...
    starterOfferAvailable,
    starterOfferExpiresAt,
    onboarding_required: user.has_completed_onboarding === false,
  };
}
...
app.post("/api/onboarding/complete", async (req, res) => {
  try {
    ...
    const currentRefs = Number(user.referrals_count || 0);
    const minRefs = ONBOARDING_MIN_REFERRALS;

    // If onboarding is already completed in the DB, just return state.
    if (user.has_completed_onboarding) {
      const state = await buildClientState(user);
      return res.json(state);
    }

    // Enforce referral-gated completion
    if (currentRefs < minRefs) {
      const state = await buildClientState(user);
      return res.json({
        ...state,
        ok: false,
        reason: "NEED_REFERRAL",
    ...
    try {
      const result = await client.query(
        "UPDATE public.users SET has_completed_onboarding = TRUE WHERE id = $1 RETURNING *",
        [user.id]
      );
    ...
    console.error("Error /api/onboarding/complete:", err);
    return res.status(500).json({ ok: false, error: "ONBOARDING_COMPLETE_ERROR" });
  }
});
...
const um = umRes.rows[0];
    ...
    if (isSponsorMission) {
      ...
      try {
        ...
        if (idemRow.status === "completed" && idemRow.response) {
          // Mission payout was already applied for this (user, code) pair.
          await client.query("COMMIT");
        ...
        const userRes = await client.query(
          `SELECT balance FROM public.users WHERE id = $1 LIMIT 1`,
          [userId]
        );
        const balance = Number(userRes.rows[0]?.balance || 0);

        const responsePayload = { ok: true, payout_amount: payout, balance };
...
app.post("/api/task", async (req, res) => {
  try {
    ...
    // Starter Bundle onboarding: start a 7-day window the first time
    // a user successfully claims the daily bonus.
    if (!user.starter_offer_purchased && !user.starter_offer_expires_at) {
      const { rows: starterRows } = await pool.query(
        `
        UPDATE public.users
        SET starter_offer_expires_at = NOW() + interval '7 days'
        WHERE id = $1
        RETURNING *;
        `,
        [user.id]
      );
      if (starterRows && starterRows[0]) {
        user = starterRows[0];
      }
    }
...
app.post("/api/referral/status", async (req, res) => {
  try {
    const user = await getOrCreateUserFromInitData(req);
    const totalInvites = Number(user.referrals_count || 0);
    const tierInfo = getReferralTier(totalInvites);

    // thresholds used for progress bar on the client
    const thresholds = [0, 1, 5, 20, 50, 200];
    const idxLevel = thresholds.findIndex((t) => totalInvites < t);
    const nextAt = idxLevel === -1 ? thresholds[thresholds.length - 1] : thresholds[idxLevel];
    const prevAt = idxLevel <= 0 ? 0 : thresholds[idxLevel - 1];
    ...
    return res.json({
      ok: true,
      tier: tierInfo.tier,
      tier_name: tierNames[tierInfo.tier] || `Tier ${tierInfo.tier}`,
...
const REFERRAL_TARGETS = [1, 5, 20, 50, 200];

async function buildReferralSummary(user) {
  const totalInvites = Number(user.referrals_count || 0);
  const totalPoints = Number(user.referrals_points || 0);
  const tierInfo = getReferralTier(totalInvites);
  const next = REFERRAL_TARGETS.find((t) => totalInvites < t) || null;

  return {
    ok: true,
    teamSize: totalInvites,
    teamEarnings: totalPoints,
    totalInvites,
    totalPoints,
    currentTier: tierInfo.tier,
    multiplier: tierInfo.multiplier,
    nextTarget: next,
    nextRemaining: next ? Math.max(0, next - totalInvites) : 0,
  };
}

// Referrals summary – POST (used by frontend)
app.post("/api/referrals/summary", async (req, res) => {
  try {
    ...
    res.status(500).json({ ok: false, error: "REFERRAL_SUMMARY_ERROR" });
  }
});

// Optional GET variant for debugging / backwards compatibility
app.get("/api/referrals/summary", async (req, res) => {
  try {
    ...
    console.error("Error (GET) /api/referrals/summary:", err);
    res.status(500).json({ ok: false, error: "REFERRAL_SUMMARY_ERROR" });
  }
});
...
async function start() {
  ...
  // Ensure Supabase schema is ready before accepting traffic.
  try {
    await ensureSchemaOnce();
  } catch (err) {
