...
// Complete a mission (grant reward on claim)
app.post("/api/mission/complete", async (req, res) => {
  try {
    ...
    if (!missionRes.rowCount) return res.status(404).json({ ok: false, error: "MISSION_NOT_FOUND_OR_INACTIVE" });
    const mission = missionRes.rows[0];

    const isSponsor = String(mission.kind || "").toLowerCase() === "sponsor" || Boolean(mission.sponsor_active);

    // Fetch existing user_mission row
    const umRes = await pool.query(
      `
      SELECT *
      FROM public.user_missions
    ...
    let um = umRes.rowCount ? umRes.rows[0] : null;

    // Cooldown check (if already claimed)
    if (um && um.claimed_at) {
    ...
    // If mission wasn't started, be resilient for sponsor missions (iOS suspend edge cases)
    if (!um) {
      if (!isSponsor) {
        return res.json({ ok: false, error: "MISSION_NOT_STARTED" });
      }
      await pool.query(
        `
        INSERT INTO public.user_missions (user_id, mission_id, status, started_at, claimed_at, reward_applied)
        VALUES ($1, $2, 'started', NOW() - (INTERVAL '1 second' * $3), NULL, FALSE)
        ON CONFLICT (user_id, mission_id)
        DO UPDATE SET status='started', started_at=NOW() - (INTERVAL '1 second' * $3), claimed_at=NULL, reward_applied=FALSE;
        `,
        [user.id, mission.id, Number(mission.min_seconds_to_claim || 30)]
      );
    ...
    // Min-seconds gate
    const startedAt = new Date(um.started_at || new Date());
    const elapsedSec = Math.floor((Date.now() - startedAt.getTime()) / 1000);
    const minSec = Number(mission.min_seconds_to_claim || 30);

    if (elapsedSec < minSec) {
      return res.json({ ok: false, error: "MISSION_NOT_READY", remaining_seconds: Math.max(0, minSec - elapsedSec) });
    }
    ...
    try {
      ...
      // Re-read row FOR UPDATE to avoid double-claims
      const umLock = await client.query(
        `SELECT * FROM public.user_missions WHERE user_id=$1 AND mission_id=$2 LIMIT 1 FOR UPDATE;`,
        [user.id, mission.id]
      );
      const umLocked = umLock.rows[0];

      if (umLocked.claimed_at) {
        // Already claimed — enforce cooldown
        const claimedAt2 = new Date(umLocked.claimed_at);
        const cooldownMs2 = Number(mission.cooldown_hours) * 60 * 60 * 1000;
        const readyAt2 = new Date(claimedAt2.getTime() + cooldownMs2);
        if (readyAt2 > new Date()) {
          const remaining2 = Math.max(0, Math.floor((readyAt2 - new Date()) / 1000));
          await client.query("ROLLBACK");
          return res.json({ ok: false, error: "MISSION_COOLDOWN", cooldown_remaining_seconds: remaining2 });
        }
      }

      // Award points once
      if (!umLocked.reward_applied) {
        await client.query(
          `UPDATE public.users SET balance = balance + $1 WHERE id = $2;`,
          [Math.floor(Number(mission.payout_amount || 0)), user.id]
        );
      }
    ...
    res.status(500).json({ ok: false, error: "MISSION_COMPLETE_ERROR", detail: String(err.message || err) });
  }
});
