...
const EARLY_ACCESS_STARTER_BONUS = Number(process.env.EARLY_ACCESS_STARTER_BONUS || 5000);
const EARLY_ACCESS_EVENT = 'early_access_starter_bonus';
const EARLY_ACCESS_LOCK_KEY = 910000001; // pg_advisory_xact_lock key
...
`);
  ...
  await client.query(`
    ...
    CREATE INDEX IF NOT EXISTS idx_userasync function getOrCreateUserFromInitData(req) {
  ...
  try {
    ...
    try {
      const existedRes = await client.query('SELECT id FROM public.users WHERE telegram_id = $1', [telegramUserId]);
      const userExisted = existedRes.rowCount > 0;
  ...
  // Upsert user by telegram_id into public.users
      ...
      // Early-access starter bonus: first N new users get +BONUS coins (idempotent)
        const now = Date.now();
        const withinWindow = !EARLY_ACCESS_END_TS || now <= EARLY_ACCESS_END_TS;
        if (withinWindow && EARLY_ACCESS_STARTER_BONUS > 0 && EARLY_ACCESS_CAP > 0) {
          // Prevent race conditions under load
          await client.query('SELECT pg_advisory_xact_lock($1)', [EARLY_ACCESS_LOCK_KEY]);

          const already = await client.query(
            'SELECT 1 FROM public.event_log WHERE user_id = $1 AND event = $2 LIMIT 1',
            [user.id, EARLY_ACCESS_EVENT]
          );

          if (already.rowCount === 0) {
            const countRes = await client.query(
              'SELECT COUNT(*)::int AS c FROM public.event_log WHERE event = $1',
              [EARLY_ACCESS_EVENT]
            );
            const awarded = countRes.rows[0]?.c ?? 0;

            if (awarded < EARLY_ACCESS_CAP) {
              await client.query(
                'UPDATE public.users SET balance = COALESCE(balance, 0) + $1 WHERE id = $2',
                [EARLY_ACCESS_STARTER_BONUS, user.id]
              );

              await client.query(
                'INSERT INTO public.event_log (user_id, event, meta) VALUES ($1, $2, $3)',
                [user.id, EARLY_ACCESS_EVENT, { telegram_id: telegramUserId, bonus: EARLY_ACCESS_STARTER_BONUS, cap: EARLY_ACCESS_CAP }]
              );

              // keep in-memory user object in sync for this response
              user.balance = (user.balance ?? 0) + EARLY_ACCESS_STARTER_BONUS;
            }
          }
        }
