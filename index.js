...
app.post("/api/referrals/summary", async (req, res) => {
  try {
    ...
    if (!user || !user.telegram_id) {
      return res.status(200).json({ ok: false, error: "NO_USER" });
    }
    ...
    const summaryRes = await pool.query(
      ...
      SELECT
        ...
        COALESCE(SUM(CASE WHEN COALESCE(u.balance, 0) > 0 THEN 1 ELSE 0 END), 0)::int AS active_invited,
        COALESCE(SUM(COALESCE(u.balance, 0)), 0)::bigint AS team_power
    ...
    const row = (summaryRes && summaryRes.rows && summaryRes.rows[0]) ? summaryRes.rows[0] : {};
    const totalInvited = Number(row.total_invited || 0);
    const activeInvited = Number(row.active_invited || 0);
    const teamPower = Number(row.team_power || 0);

    const refCount = Number(user.referrals_count || 0);
    const refPoints = Number(user.referrals_points || 0);

    let nextMilestone = 0;
    if (refCount < 1) nextMilestone = 1;
    else if (refCount < 5) nextMilestone = 5;
    else if (refCount < 20) nextMilestone = 20;
    else if (refCount < 50) nextMilestone = 50;
    else if (refCount < 200) nextMilestone = 200;
    else nextMilestone = refCount;

    return res.json({
      ok: true,
      team_size: totalInvited,
      active_team_size: activeInvited,
      team_power: teamPower,
      referrals_count: refCount,
      referrals_points: refPoints,
      next_milestone: nextMilestone,
    });
  } catch (e) {
    console.error("Error /api/referrals/summary:", e);
    return res.status(200).json({ ok: false, error: "REFERRAL_SUMMARY_ERROR" });
  }
});
