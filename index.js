...
app.post("/api/mission/complete", async (req, res) => {
  try {
    ...
    // Enforce cooldown for repeatable missions (e.g. sponsor quests)
    const existingUM = await pool.query(
      `SELECT id, status, started_at, completed_at, reward_applied FROM public.user_missions WHERE user_id = $1 AND mission_id = $2 LIMIT 1;`,
      [user.id, mission.id]
    );
        // Sponsor quests require an explicit Start before Claim
    if ((mission.kind || "") === "sponsor") {
      const um0 = existingUM.rowCount > 0 ? existingUM.rows[0] : null;
      if (!um0 || (um0.status || "") !== "started") {
        return res.json({ ok: false, error: "MUST_START_SPONSOR" });
      }
    }

const remaining = missionCooldownRemainingSeconds(mission, user, existingUM.rows[0]);
    if (remaining > 0) {
      return res.json({
        ok: false,
        error: "MISSION_COOLDOWN",
        cooldown_seconds: Number(mission.cooldown_seconds || 0),
    ...
    if (cooldownSeconds > 0) {
      // Repeatable: reset reward_applied each cycle and bump completed_at
      umRes = await pool.query(
        `
    ...
    // Strong anti-farm: must Start first
    if (!userMission || (userMission.status || "") !== "started") {
      return res.status(400).json({ ok: false, error: "MISSION_NOT_STARTED" });
    }

    // Per-mission cooldown (if somehow completed already)
    const cd = missionCooldownRemainingSeconds(mission, user, userMission);
    if (cd > 0) {
      return res.status(429).json({ ok: false, error: "MISSION_COOLDOWN", cooldown_seconds: cd });
    }

    // Wait gate
    const wait = missionWaitRemainingSeconds(mission, userMission);
    if (wait > 0) {
      return res.status(429).json({ ok: false, error: "MISSION_TOO_FAST", wait_seconds: wait });
    }
    ...
    // Global sponsor cooldown marker (one sponsor quest per 24h)
    if ((mission.kind || "") === "sponsor") {
      await pool.query(`UPDATE public.users SET last_sponsor_ts = NOW() WHERE id = $1;`, [user.id]);
      user.last_sponsor_ts = new Date().toISOString();
    }
