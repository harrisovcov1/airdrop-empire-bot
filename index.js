...
app.post("/api/tapPacket", async (req, res) => {
  const client = await pool.connect();
  try {
    // Validate & authenticate first (keep behavior identical)
    let user = await getOrCreateUserFromInitData(req);
    ...
    // Idempotency: if a client sends the same packet twice, process it only once.
    if (packetId) {
      try {
        ...
        // If tap_packets table doesn't exist, fail open (no idempotency).
        console.warn("tap_packets insert failed (continuing):", e.message || e);
      }
    }

    // Lock the user row to prevent concurrent packets from double-spending energy.
    const lockedRes = await client.query(
      `SELECT * FROM public.users WHERE id = $1 FOR UPDATE;`,
      [user.id]
    );
    let u = lockedRes.rows[0] || user;

    // --- Apply daily reset (same logic as ensureDailyReset, but inside this TX) ---
    const today = todayDate();
    let lastResetStr = null;
    try {
      if (u.last_reset) {
        if (typeof u.last_reset === "string") {
          lastResetStr = u.last_reset.slice(0, 10);
        } else {
          const tmp = new Date(u.last_reset);
          if (!isNaN(tmp)) lastResetStr = tmp.toISOString().slice(0, 10);
        }
      }
    } catch (e) {
      console.error("Bad last_reset:", u.last_reset, e);
      lastResetStr = null;
    }
    ...
    // --- Apply energy regen (same logic as applyEnergyRegen, but inside this TX) ---
    const maxEnergy = u.max_energy || 50;
    const now = new Date();

    let lastEnergyTs = u.last_energy_ts ? new Date(u.last_energy_ts) : null;
    if (!lastEnergyTs || isNaN(lastEnergyTs)) {
      lastEnergyTs = new Date(now.getTime() - 60 * 60 * 1000);
    }

    let diffSeconds = Math.floor((now - lastEnergyTs) / 1000);
    if (diffSeconds > 0) {
      let energy = Number(u.energy || 0);

      while (diffSeconds > 0 && energy < maxEnergy) {
        let step;
        if (energy < 10) step = 1;
        else if (energy < 30) step = 3;
        else step = 6;
    ...
    const currentEnergy = Number(u.energy || 0);
    if (currentEnergy <= 0) {
      await client.query("COMMIT");
      client.release();
      const state = await buildClientState(u);
      return res.json({ ...state, ok: false, reason: "NO_ENERGY" });
    }

    const vipActive = u.vip_until && new Date(u.vip_until) > new Date();
    const maxTapsPerDay = vipActive ? VIP_DAILY_TAP_CAP : 5000;
    const currentTaps = Number(u.taps_today || 0);
    if (currentTaps >= maxTapsPerDay) {
      await client.query("COMMIT");
      client.release();
      const state = await buildClientState(u);
    ...
    // IMPORTANT: This TX lock makes allowedByEnergy authoritative (prevents "extra taps")
    const n = Math.max(1, Math.min(count, allowedByCap, allowedByEnergy));

    const basePerTap = 1;
    let perTap = basePerTap;
    if (u.double_boost_until) {
      const until = new Date(u.double_boost_until);
  ...
  try {
    ...
    if (isSponsorMission) {
      ...
      try {
        ...
        await client.query("COMMIT");
        return res.json({ ok: true, payout_amount: payout });
      } catch (e) {
