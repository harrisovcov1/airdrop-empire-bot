...
async function applyEnergyRegen(user) {
  ...
  let diffMs = now - last;
  if (!Number.isFinite(diffMs) || diffMs <= 0) return user;

  let energy = Number(user.energy || 0);

  // Industrial-scale regen rhythm (≈ 60–90s from 0 → full on maxEnergy=50)
  // 0–10: fast, 10–20: a bit slower, 20–30: slower, 30+: slowest.
  while (diffMs > 0 && energy < maxEnergy) {
    let stepMs;

    if (energy < 10) stepMs = 250;        // 0–10 fast
    else if (energy < 20) stepMs = 800;   // 10–20 a bit slower
    else if (energy < 30) stepMs = 1200;  // 20–30 slower
    else stepMs = 1800;                   // 30+ slowest
