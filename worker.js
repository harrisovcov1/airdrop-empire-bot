...
const { applyBalanceChangeTx } = require("./lib/ledger");
...
async function processJob(job) {
  await withTransaction(async (client) => {
    ...
    try {
      if (job.type === "withdraw_payout") {
        await handleWithdrawPayout(client, job);
      } else {
      ...
      await markJobStatus(client, job.id, finalStatus, err.message || String(err));
    }
  });
}
